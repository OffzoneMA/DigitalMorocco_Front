...
export const sponsorApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: import.meta.env.VITE_baseURL + "/sponsors",
  ...
  endpoints: (builder) => ({
    ...
    getDistinctEventFieldsByPartner: builder.query({
      query: ({ field, eventStatus, sponsorStatus }) =>
        `/partner/distinct?field=${field}${
          eventStatus ? `&eventStatus=${eventStatus}` : ""
        }${sponsorStatus ? `&sponsorStatus=${sponsorStatus}` : ""}`,
      providesTags: ["Event"],
    }),
    getDistinctEventFieldsByPartnerHistory: builder.query({
      query: ({ field, eventStatus, sponsorStatus }) =>
        `/partner/history/distinct?field=${field}${
          eventStatus ? `&eventStatus=${eventStatus}` : ""
        }${sponsorStatus ? `&sponsorStatus=${sponsorStatus}` : ""}`,
      providesTags: ["Event"],
    }),
...
export const {
  ...
  useGetApprovedSponsorsForPastEventsQuery,
