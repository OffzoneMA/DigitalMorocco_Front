...
const PastEvents = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAllPastEventsUserParticipateQuery({
    page: currentPage,
  ...
  const displayedEvents =
    userDetails?.email?.toLowerCase() === "contact.vfhl@gmail.com"
      ? eventsDT?.events
      : [];
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/PastEvent`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("event.pastEvent")}</PageHeader>
            </div>
          ...
          <div className="flex flex-col items-start justify-start w-full min-h-[440px]">
            {displayedEvents?.length > 0 ? (
              displayedEvents.map((item, index) => (
                <div
                  key={index}
                  className="cursorpointer group flex flex-col md:flex-row gap-3 w-full bg-white-A700 border-b border-gray-201 py-6"
                  onClick={() =>
                    navigate(`/PastEventDetails/${item?._id}`, {
                      state: { event: item },
                    })
                  }
                >
                  ...
                  <div className="flex flex-col flex-1 gap-3">
                    <Text className="font-dm-sans-medium text-lg leading-7 text-left text-blue_gray-903 w-full group-hover:text-[#00CDAE]">
                      {item?.title}
                    </Text>
                    <div className="flex flex-row gap-3 items-center text-left">
                      ...
                      />
                      <Text className="text-blue_gray-601 font-dm-sans-regular text-base leading-6">
                        {formatEventDateTime(
                          item?.startDate,
                          item?.endDate,
