                    ...
                    <tbody className="items-center w-full ">
                      {
                        (pageData.map((item, index) => (
                          <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-50' : ''} hover:bg-blue-50 w-full cursorpointer`} onClick={() => navigate(`/EventDetails/${item?._id}`)} >
                            ...
                            <td className="px-[18px] py-[14px] text-gray-801 font-dm-sans-regular text-sm leading-relaxed"
                              style={{ whiteSpace: "nowrap" }}>
                              <div style={{ whiteSpace: "nowrap", textDecoration: 'capitalize' }} className={`flex flex-row space-x-2 items-center px-2 capitalize font-dm-sans-regular text-sm leading-6 text-white-A700 rounded-full ${item.status?.toLowerCase() === 'past' ? 'bg-[#F4A118]' :
              ...
              : (
                isLoading ?
                  ...
                  <div className="flex flex-col items-center h-screen w-full py-28 gap-[16px] ">
                    <img src={ticketEmptyImg} alt="No Tickets" />
                    <p
