...
const ManageSubscriptionCredits = () => {
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:px-5 px-8 gap-8 w-full h-full">
          ...
          <div className="h-full w-full items-start gap-[42px] flex flex-col md:flex-row flex-wrap pb-[80px]">
            ...
            <form
              ...
              className="min-w-auto max-w-[339px] md:w-[339px] min-h-[481px] p-6 bg-white-A700 rounded-xl border border-[#e4e6eb] flex-col justify-start items-start gap-6 flex"
            >
              ...
              {subscriptionData?.plan &&
                ["Standard", "Standard In", "Premium"].includes(
              ...
              {subscriptionData?.plan &&
                ["Standard", "Standard In", "Premium"].includes(
                  ...
                  <div className="self-stretch justify-between items-start flex">
                    <div className="text-[#98a1b2] text-base font-dm-sans-medium">
                      {t("Discount")} {` `} {`(`}{" "}
                      {subscriptionData?.plan?.forUser?.toLowerCase() ===
                      "investor"
                        ? t(
                            `subscriptionPlans.investor.${subscriptionData?.plan?.name.toLowerCase()}.name`
                          )
                        : t(
                            `subscriptionPlans.${subscriptionData?.plan?.name.toLowerCase()}.name`
                          )}{" "}
                      {`)`}{" "}
                    </div>
                    <div className="text-[#1e0d62] text-lg font-dm-sans-medium leading-7">
                      {subscriptionData?.plan?.discountRate}%
                    </div>
                  </div>
                )}
              <div className="self-stretch h-[0px] border-2 border-[#eaeaed]"></div>
              <div className="self-stretch justify-between items-end flex">
                ...
                <div className="text-[#1e0d62] text-[22px] font-dm-sans-bold leading-7">
                  $ {calculateTotalPrice(selectedCredits?.price?.toFixed(2))}
                </div>
              </div>
      ...
      />
      <EmailExistModalOrConfirmation
        isOpen={isCancelPaidModalOpen}
        onRequestClose={closeCancelPaidModal}
        content={
          <div className="flex flex-col gap-[38px] items-center justify-start  w-full">
            <img
              className="h-[80px] w-[80px]"
