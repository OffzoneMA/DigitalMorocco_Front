...
export default function SubscribePlan() {
  ...
  const [
    upgradeSubscription,
    { isLoading: upgradeLoading, isSuccess: upgradeSuccess, isError, error },
  ] = useUpgradeSubscriptionMutation();
  ...
  const annualPrice = formatPrice(
    choosedPlan?.annualPrice || (choosedPlan?.price * 12).toFixed(2)
  );
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/SubscribePlan`}
      />
              ...
              <div className="flex flex-col w-full rounded-[12px]  border p-4 border-gray-301 gap-4">
                <div className="flex flex-col w-full border-b pb-4 border-gray-301 gap-4">
                  ...
                  <div className="flex flex-col w-full rounded-[12px] border border-[#E4E7EC] gap-[24px] p-4 ">
                    <div className="flex flex-col gap-1 w-full">
                      <div className="flex flex-row w-full items-center">
                        <Text className="font-dm-sans-medium text-[22px] leading-8 text-left text-blue-501">
                          {choosedPlan?.forUser?.toLowerCase() === "investor"
                            ? t(
                                `subscriptionPlans.investor.${choosedPlan?.name?.toLowerCase()}.name`
                              )
                            : t(
                                `subscriptionPlans.${choosedPlan?.name?.toLowerCase()}.name`
                              )}
                        </Text>
                        <div className="flex flex-row ml-auto">
                          <button
                            className={`flex justify-center items-center gap-3 w-[150px] h-[44px] p-[18px_30px] rounded-md hover:bg-[#235DBD] active:bg-[#224a94] cursorpointer ${
                              selectedPlan === "Monthly"
                                ? "bg-blue-A400 text-white-A700"
                                : "border border-blue-A400 text-blue-A400 hover:text-[#EDF7FF]"
                            }`}
                            onClick={() => setSelectedPlan("Monthly")}
                            type="button"
                          >
                            <span className="text-base leading-[20.83px] font-dm-sans-medium">
                              {t("subscriptionPlans.billingCycle.monthly")}
                            </span>
                          </button>
                          <button
                          ...
                          >
                            <span className="text-base leading-[20.83px] font-medium">
                              {t("subscriptionPlans.billingCycle.annual")}
                            </span>
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-[16px]">
                      {choosedPlan?.featureDescriptions?.map(
                        (feature, index) => (
                          <div
                            key={index}
                            className={`flex flex-row w-full items-start gap-[12px]`}
                          >
                              ...
                              {formatPrice(
                                (
                                  (annualPriceNotFormat *
                                    (100 -
                                      (choosedPlan?.annualDiscountRate ||
                                        20))) /
                                  100
                                ).toFixed(2)
                              )}
                        ...
                        <div className="inline-flex h-[24px] p-[2px_10px] justify-center items-center inline-flex rounded-[6px] bg-[#E1FFED] ml-6">
                          <span className="text-[#00CDAE] text-center font-dm-sans text-[10px] font-bold leading-[24px]">
                            {t("settings.subscription.annualDiscount", {
                              rate: choosedPlan?.annualDiscountRate || 20,
                            })}
                          </span>
                        </div>
                      </div>
                ...
                <div className="flex space-x-3 md:space-x-5 items-end  w-full py-2 justify-end">
                  ...
                  <button
                    ...
                    } hover:bg-[#235DBD] active:bg-[#224a94] text-base leading-[20.83px] w-[282px] h-11 px-[30px] py-[18px] font-dm-sans-medium text-white-A700 flex flex-row items-center justify-center tracking-normal gap-3 ml-auto py-3 px-5 rounded-md w-auto cursorpointer`}
                    type="button"
        ...
        content={
          <div className="flex flex-col gap-[38px] items-center justify-start  w-full">
            ...
            <div className="flex flex-col gap-5 items-center justify-start w-full">
              <Text className="text-[#1d2838] w-[460px] text-lg leading-relaxed font-dm-sans-medium text-center ">
                {t("subscriptionPlans.confirmSuccess.title")}
              </Text>
              <Text className="leading-relaxed w-[460px] font-dm-sans-regular text-[#1d2838] text-center text-sm">
