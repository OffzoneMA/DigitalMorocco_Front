...
export default function Subscription() {
  ...
  const [renewSubscriptionLoading, setRenewSubscriptionLoading] =
    useState(false);
  ...
  const fetchLastPaymentMethod = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_baseURL}/payment-methods/last`,
        {
  ...
  useEffect(() => {
    if (userLastPaymentMethod) {
      const method = paymentMethodsData.find(
        (method) => method.name === userLastPaymentMethod.paymentMethod
      );
  ...
  const getUserSusbcription = async () => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_baseURL}/subscriptions/forUser`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const annualPrice = formatPrice(
    userSubscriptionData?.plan?.annualPrice ||
      (userSubscriptionData?.plan?.price * 12).toFixed(2)
  );
  const annualPriceNotFormat =
    userSubscriptionData?.plan?.annualPrice ||
    (userSubscriptionData?.plan?.price * 12).toFixed(2);
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/Subscription`}
      />
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen overflow-auto items-start justify-start pb-14 pt-8 rounded-tl-[40px] w-full">
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("settings.subscription.title")}</PageHeader>
            </div>
          </div>
              ...
              {!isSubscribe && !isSubscribeLoading ? (
                <div className="flex flex-col w-full lg:flex-1 gap-4">
                  ...
                  <div className="flex flex-col w-full rounded-[6px] border p-5 border-gray-301 gap-5">
                    ...
                    >
                      <path
                        d="M18 12V18M18 24H18.015M4.5 11.9122V24.0878C4.5 24.6018 4.5 24.8588 4.57573 25.088C4.64272 25.2907 4.75224 25.4769 4.89695 25.6339C5.06053 25.8114 5.28517 25.9362 5.73446 26.1858L16.8345 32.3525C17.2599 32.5888 17.4726 32.707 17.6978 32.7533C17.8972 32.7943 18.1028 32.7943 18.3022 32.7533C18.5274 32.707 18.7401 32.5888 19.1655 32.3525L30.2655 26.1858C30.7148 25.9362 30.9395 25.8114 31.1031 25.6339C31.2478 25.4769 31.3573 25.2907 31.4243 25.088C31.5 24.8588 31.5 24.6018 31.5 24.0878V11.9122C31.5 11.3982 31.5 11.1412 31.4243 10.912C31.3573 10.7093 31.2478 10.5231 31.1031 10.3661C30.9395 10.1886 30.7148 10.0638 30.2655 9.81419L19.1655 3.64753C18.7401 3.41119 18.5274 3.29302 18.3022 3.24669C18.1028 3.20569 17.8972 3.20569 17.6978 3.24669C17.4726 3.29302 17.2599 3.41119 16.8345 3.64753L5.73446 9.81419C5.28517 10.0638 5.06053 10.1886 4.89695 10.3661C4.75224 10.5231 4.64272 10.7093 4.57573 10.912C4.5 11.1412 4.5 11.3982 4.5 11.9122Z"
                        stroke="#DB7712"
                        strokeWidth="3"
                        strokeLinecap="round"
                    ...
                    <Text className="font-dm-sans-bold text-base leading-6 text-left text-col1 w-full">
                      {userData?.role?.toLowerCase() === "member"
                        ? t("settings.subscription.subscriptionPromo")
                        : t("settings.subscription.subscriptionOffer")}
                    </Text>
                    ...
                    <Text className="font-dm-sans-bold text-base leading-6 text-left w-full text-[#101828]">
                      {userData?.role?.toLowerCase() === "member"
                        ...
                        : t("settings.subscription.subscriptionCtaIn")}
                    </Text>
                    <button
                    ...
                    >
                      ...
                      {userData?.role?.toLowerCase() === "member"
                        ...
                        : t("settings.subscription.startFree")}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col w-full lg:flex-1 gap-4">
                  ...
                  <div className="flex flex-col w-full rounded-[12px] border p-5 border-gray-301 gap-4">
                      ...
                      <div className="flex flex-col w-full rounded-[12px] border p-[24px] border-gray-301 gap-[24px]">
                        <div className="flex flex-row w-full items-center pb-1">
                          <Text className="font-dm-sans-medium text-[22px] leading-8 text-left text-blue-501">
                            {userSubscriptionData?.plan?.forUser?.toLowerCase() ===
                            "investor"
                              ? t(
                                  `subscriptionPlans.investor.${userSubscriptionData?.plan?.name.toLowerCase()}.name`
                                )
                              : t(
                                  `subscriptionPlans.${userSubscriptionData?.plan?.name.toLowerCase()}.name`
                                )}
                          </Text>
                          <button
                            onClick={() => navigate("/ChoosePlan")}
                        ...
                        <div className="flex flex-col w-full gap-[16px]">
                          {userSubscriptionData?.plan?.featureDescriptions?.map(
                            (feature, index) => (
                              <div
                                key={index}
                                className="flex flex-rox w-full items-start gap-[12px]"
                              >
                        ...
                        userSubscriptionData?.plan?.price > 0 &&
                        userSubscriptionData?.planType !== "upcoming" ? (
                          <div className="flex flex-row items-center w-full pt-1">
                            ...
                            {userSubscriptionData?.plan?.price > 0 &&
                              userSubscriptionData?.planType !== "upcoming" && (
                                <Text className="font-dm-sans-medium text-lg leading-8 text-left text-[#1D2939] ml-2">
                                  ...
                                  , {t("subscriptionPlans.endsOn")}{" "}
                                  {getEndDate(annualDuration)}
                                </Text>
                              )}
                            <div className="inline-flex h-[24px] p-[2px_10px] justify-center items-center inline-flex rounded-[6px] bg-[#E1FFED] ml-6">
                              <span className="text-[#00CDAE] text-center font-dm-sans text-[10px] font-bold leading-[24px]">
                                {t("settings.subscription.annualDiscount", {
                                  rate:
                                    userSubscriptionData?.plan
                                      ?.annualDiscountRate || 20,
                                })}
                          ...
                          userSubscriptionData?.planType !== "upcoming" && (
                            <Text className="font-dm-sans-medium text-lg leading-8 pt-1 text-left w-full text-[#1D2939]">
                              {monthlyPrice}/{t("subscriptionPlans.monthlyFee")}
                              , {t("subscriptionPlans.endsOn")}{" "}
                              {getEndDate(monthlyDuration)}
                            </Text>
                          )
                        )}
                      </div>
                    ...
                    <div className="flex flex-col w-full border-t pt-5 border-gray-301 gap-5">
                      <div className="w-auto text-[#101828] text-lg font-dm-sans-medium leading-7">
                        {t("settings.subscription.billingInformation")}
                      </div>
                      <table className="w-full h-auto bg-white">
                        <thead>
                          <tr className="text-left text-[#344054] font-DmSans font-medium h-[44px]">
                            <th className="h-[44px] px-[18px] py-2 bg-white text-sm font-DmSans font-medium leading-relaxed">
                              {t("settings.subscription.uploadDate")}
                            </th>
                            <th className="h-[44px] px-[18px] py-2 bg-white text-sm font-DmSans font-medium leading-relaxed">
                              {t("settings.subscription.documentName")}
                            </th>
                            <th className="h-[44px] px-[18px] py-2 bg-white text-sm font-DmSans font-medium leading-relaxed">
                              {t("settings.subscription.status")}
                            </th>
                            <th className="h-[44px] px-[18px] py-2 bg-white text-sm font-DmSans font-medium leading-relaxed">
                              {t("settings.subscription.action")}
                            </th>
                          </tr>
                        </thead>
                        {!billingDataFetching &&
                                  ...
                                  <div className="flex w-full items-center">
                                    {item?.status && (
                                      <div
                                        ...
                                        className={`px-[10px] h-[28px] py-[2px] flex items-center justify-center w-auto rounded-[16px] text-center text-[13px] font-dm-sans-regular leading-normal ${
                                          item?.status?.toLowerCase() ===
                                          "upcoming"
                                            ? "bg-[#DBEEFF] text-[#1570EF]"
                                            : item?.status?.toLowerCase() ===
                                              "paid"
                                            ? "bg-[#ECFDF3] text-[#027A48]"
                                            : "bg-[#FEE8E6] text-[#F04438]"
                                        }`}
                                      >
                                  ...
                                  <div className="flex items-center gap-[18px]">
                                    ...
                                    <div className="relative group">
                                      ...
                                      >
                                        ...
                                        />
                                        <path
                                          d="M9.0003 8.25C10.2429 8.25 11.2503 7.24264 11.2503 6C11.2503 4.75736 10.2429 3.75 9.0003 3.75C7.75766 3.75 6.7503 4.75736 6.7503 6C6.7503 7.24264 7.75766 8.25 9.0003 8.25Z"
                                          stroke="#98A2B3"
                                          strokeWidth="1.4"
                                          strokeLinecap="round"
                                          strokeLinejoin="round"
        ...
        <CancelPlanModal
          ...
          response={response}
          refetch={getUserSusbcription}
        />
        <EmailExistModalOrConfirmation
          isOpen={isConfirmRenewModalOpen}
          onRequestClose={closeConfirmRenewModal}
          content={
            <div className="flex flex-col gap-[38px] items-center justify-start w-full">
              ...
              <div className="flex flex-col gap-5 items-center justify-start w-full">
                <Text className="leading-[26.00px] font-dm-sans-medium text-[18px] text-[#1d2838] text-center ">
                  {t("subscriptionPlans.confirmSuccess.renewTitle")}
                </Text>
                <Text className="leading-[26.00px] font-dm-sans-regular text-[#1d2838] text-center text-sm">
