...
export default function Subscription() {
  ...
  // Open modal based on the statuspaid value and subscriptionType
  useEffect(() => {
    if (statuspaid === "success") {
      setOpenSuccessPaidModal(true);
    } else if (statuspaid === "failed") {
      setOpenErrorPaidModal(true);
    }
  }, [statuspaid]);
  ...
  const handleRenewSubscription = async () => {
    ...
    // Store subscription type in local storage
    localStorage.setItem("subscriptionType", 'renew');
    try {
      const response = await renewSubscription(userSubscriptionData?._id).unwrap();

      if (response?.isSuccess || response?.success) {
        const { paywallUrl, payload, signature } = response.data;
  ...
  const closeSelectProjectToMaskModal = () => {
    // setOpenChooseProjectToMaskModal(false);
    localStorage.removeItem("subscriptionType");
    navigate("/projects");
  };
  ...
  return (
    <>
      ...
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen overflow-auto items-start justify-start pb-14 pt-8 rounded-tl-[40px] w-full">
              ...
              {(!isSubscribe && !isSubscribeLoading) ?
                (
                  <div className="flex flex-col w-full lg:flex-1 gap-4">
                    <h1 className="font-dm-sans-medium text-lg leading-7 text-[#101828] text-left w-full">
                    ...
                    <div className="flex flex-col w-full rounded-[6px] border p-5 border-gray-301 gap-5">
                      ...
                      <p className="font-dm-sans-bold text-base leading-6 text-left w-full text-[#101828]">
                        {t('settings.subscription.noSubscription')}
                      </p>
                      <p className="font-dm-sans-bold text-base leading-6 text-left text-col1 w-full">
                        {userData?.role?.toLowerCase() === "member" ? t('settings.subscription.subscriptionPromo') : t('settings.subscription.subscriptionOffer')}
                      </p>
                      <p className="font-dm-sans-regular text-sm leading-[22.4px] text-[#1D2939] text-left w-full">
                  ...
                  <div className="flex flex-col w-full lg:flex-1 gap-4">
                    ...
                    <div className="flex flex-col w-full rounded-[12px] border p-5 border-gray-301 gap-4">
                      <div className="flex flex-col w-full border-b pb-5 border-gray-301 gap-[18px] ">
                        <h2 className="font-dm-sans-bold text-base leading-6 text-left w-full text-[#101828]">
                        ...
                        <div className="flex flex-col w-full rounded-[12px] border p-[24px] border-gray-301 gap-[24px]">
                          <div className="flex flex-row w-full items-center pb-1">
                            <h2 className="font-dm-sans-medium text-[22px] leading-8 text-left text-blue-501">
                          ...
                          <div className="flex flex-col w-full gap-[16px]">
                            {userSubscriptionData?.plan?.featureDescriptions?.map((feature, index) => (
                              <div key={index} className="flex flex-rox w-full items-start gap-[12px]">
                                ...
                                <p className="font-dm-sans-regular text-base leading-6 text-left w-full text-gray700">
                                  {t(feature)}
                                </p>
                              </div>
                            ))}
                          ...
                          {(userSubscriptionData?.billing === "year" && userSubscriptionData?.plan?.price > 0 && userSubscriptionData?.planType !== "upcoming") ? (
                            <div className="flex flex-row items-center w-full pt-1">
                              <p className="font-dm-sans-medium text-lg leading-8 text-left text-gray-500 line-through">
                                {annualPrice}
                              </p>
                              {(userSubscriptionData?.plan?.price > 0 && userSubscriptionData?.planType !== "upcoming") && <p className="font-dm-sans-medium text-lg leading-8 text-left text-[#1D2939] ml-2">
                            ...
                            (userSubscriptionData?.plan?.price > 0 && userSubscriptionData?.planType !== "upcoming") && <p className="font-dm-sans-medium text-lg leading-8 pt-1 text-left w-full text-[#1D2939]">
                        ...
                        {showNotification && <>
                          ...
                          <p className="font-dm-sans-regular text-sm leading-[19.2px] text-[#1D2939] text-left w-full">
                                  ...
                                  <td className="h-16 px-[18px] py-4">
                                    <div className="flex items-center gap-[18px]">
                                      <div className="relative group">
                                        <a href={item?.document?.link} download={item?.document?.name} target="_blank" rel="noopener noreferrer">
        ...
        <EmailExistModalOrConfirmation isOpen={openSuccessPaidModal}
          onRequestClose={closeOpenSuccessPaidModal} content={
            <div className="flex flex-col gap-[38px] items-center justify-start  w-full">
              ...
              <div className="flex flex-col gap-5 items-center justify-start w-full">
                ...
                >
                  {subscriptionType === 'new' ? t('subscriptionPlans.confirmSuccess.title') : subscriptionType === 'renew' ? t('subscriptionPlans.confirmSuccess.renewTitle') : t('subscriptionPlans.confirmSuccess.upgradeTitle')}
                </h1>
                <p
        ...
        <EmailExistModalOrConfirmation isOpen={openErrorPaidModal}
          onRequestClose={closeOpenErrorPaidModal} content={
            <div className="flex flex-col gap-[38px] items-center justify-start  w-full">
              <img
                className="h-[80px] w-[80px]"
                src={emailErrorImg}
              ...
              <div className="flex flex-col gap-5 items-center justify-start w-full">
                <h1
                  className="text-[#1d2838] w-[460px] text-lg leading-relaxed font-dm-sans-medium text-center "
                >
                  {t('subscriptionPlans.paidError.title')}
                </h1>
                <p
                  className="leading-relaxed w-[460px] font-dm-sans-regular text-[#1d2838] text-center text-sm"
                >
                  <>
                    {t('subscriptionPlans.paidError.message')}
                  </>
                </p>
              </div>
            </div>
