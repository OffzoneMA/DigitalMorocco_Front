...
const NewEmployee = () => {
  ...
  const [selectedCountry, setSelectedCountry] = useState(
    dataCountries.find((country) => country.name === employee?.country) || null
  );
  const [selectedCity, setSelectedCity] = useState(employee?.cityState || "");
  const [selectedJobTitle, setSelectedJobTitle] = useState(
    jobTitles.find((job) => job === employee?.jobTitle) || null
  );
  ...
  useEffect(() => {
    if (location.state && location.state.employee) {
      ...
      if (location?.state?.employee?.jobTitle) {
        setSelectedJobTitle(
          jobTitles.find((job) => job === location.state.employee.jobTitle)
        );
      }
      if (location.state?.employee?.level) {
        setSelectedLevel(
          employeeLevels.find(
            (level) => level === location.state.employee.level
          )
        );
      }
      if (location.state?.employee?.department) {
        setSelectedDepartment(
          departments.find(
            (department) => department === location.state.employee.department
          )
        );
      }
    }
  ...
  const handleRemoveLogo = async () => {
    ...
    if (!imgFile && employee?.image) {
      const response = await axios.delete(
        `${import.meta.env.VITE_baseURL}/employee/${employeeId}/deleteImage`,
        {
          headers: {
    ...
    try {
      ...
      if (
        ...
        if (imgFile) {
          ...
          updatedFields.startDate = selectedDate
          ...
          if (employeeId) {
            ...
            const response = await axios.put(
              `${import.meta.env.VITE_baseURL}/employee/${employeeId}`,
              formData,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
            ...
            if (response?.ok) {
              ...
              response
                ...
                .catch((error) => {
                  console.error(
                    "Erreur lors de la lecture du corps de la réponse :",
                    error
                  );
                });
            }
            ...
            const response = await fetch(
              `${import.meta.env.VITE_baseURL}/employee/add`,
              {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${token}`,
            ...
            if (response?.ok) {
              response
                ...
                .then((data) => {
                  ...
                  setTimeout(() => {
                    setIsSaved(false);
                    navigate(`/EditEmployee/${data?._id}`, {
                      state: { employee: data },
                    });
                  }, 2000);
                })
                .catch((error) => {
                  console.error(
                    "Erreur lors de la lecture des données de la réponse :",
                    error
                  );
                });
            }
          ...
          if (employeeId) {
            ...
            if (response?.data?._id) {
              ...
              navigate(`/EditEmployee/${response?.data?._id}`, {
                state: { employee: response.data },
              });
            }
          }
  ...
  //         await axios.put(`${import.meta.env.VITE_baseURL}/members/${userId}/employees/${employeeId}`, requestData, {
  //           headers: {
  //             Authorization: `Bearer ${token}`,
  ...
  //       else{
  //         fetch(`${import.meta.env.VITE_baseURL}/members/employees/${userId}`, {
  //         method: 'POST',
  //         headers: {
  //           Authorization: `Bearer ${token}`,
  //           'Content-Type': 'application/json',
  ...
  return (
    <>
      <HelmetWrapper
        title={
          !employeeId
            ? t("helmet.company.employees.create.title")
            : t("helmet.company.employees.edit.title")
        }
        description={
          !employeeId
            ? t("helmet.company.employees.create.description")
            : t("helmet.company.employees.edit.description")
        }
        keywords={
          !employeeId
            ? t("helmet.company.employees.create.keywords")
            : t("helmet.company.employees.edit.keywords")
        }
        canonical={
          !employeeId
            ? `${import.meta.env.VITE_URL}/CreateEmployee`
            : `${import.meta.env.VITE_URL}/EditEmployee/${employeeId}`
        }
      />
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen items-start justify-start pb-8 pt-8 rounded-tl-[40px] overflow-y-auto w-full">
        ...
        <div className="flex items-start justify-start w-full">
          <div
            className="flex flex-col items-start justify-start sm:px-5 px-8 w-full"
            style={{ backgroundColor: "white" }}
          >
            <form
              className="w-full border border-gray-201 rounded-[8px] shadow-tablesbs "
            ...
            >
              <div className="flex flex-row flex-wrap text-sm text-center text-gray-300 border-b border-gray-201 rounded-t-lg bg-white-A700 h-[77px] py-[19px] px-5">
                ...
                >
                  {employee
                    ? t("employee.addEmployee.editEmployee")
                    : t("employee.addEmployee.addEmployee")}
                </Text>
                {isSaved ? (
                  <button
              ...
              <div className="flex flex-col flex-wrap md:flex-row lg:flex-row xl:flex-row 3xl:flex-row 2xl:flex-row gap-8 items-start justify-start px-5 md:px-5 w-full">
                <div className="flex min-w-[300px] flex-1 flex-col gap-6 py-5 items-start justify-start w-full">
                  ...
                  >
                    ...
                    <input
                      {...register("workEmail", {
                        required: {
                          value: true,
                          message: "Employee Work Email is required",
                        },
                        minLength: {
                          value: 2,
                        },
                  ...
                  >
                    ...
                    <input
                      {...register("personalEmail", {
                        required: {
                          value: false,
                          message: "Employee Personal Email is required",
                        },
                        minLength: {
                          value: 2,
                        },
                  ...
                  >
                    ...
                    <SimpleSelect
                      ...
                      placeholder={t("employee.addEmployee.selectCountry")}
                      valuekey="name"
                  ...
                  >
                    ...
                    <input
                      {...register("personalTaxIdentifierNumber", {
                        required: {
                          value: false,
                          message:
                            "Employee Personal Tax Identifier Number is required",
                        },
                      })}
                      className={`!placeholder:text-blue_gray-301 !text-gray700 leading-[18.2px] font-manrope text-left text-sm tracking-[0.14px] w-full rounded-[6px] px-[12px] py-[10px] h-[40px] border ${
                        errors?.personalTaxIdentifierNumber
                          ? "border-errorColor shadow-inputBsError focus:border-errorColor"
                          : "border-[#D0D5DD] focus:border-focusColor focus:shadow-inputBs"
                      }`}
                      type="text"
                      ...
                      onChange={(e) =>
                        handleChange(e, "personalTaxIdentifierNumber")
                      }
                ...
                <div className="flex py-5 flex-col items-start justify-start w-full lg:w-[35%] xl:w-[35%] 2xl:w-[35%] 3xl:w-[35%]">
                  <div className="flex flex-col gap-6 items-start justify-start w-full">
                    <div
                      className="bg-white-A700 border border-blue_gray-100_01 border-solid h-[270px] flex flex-col items-center justify-center relative rounded-md w-full"
                    ...
                    >
                      ...
                      <SimpleSelect
                        ...
                        searchLabel={t("employee.addEmployee.searchLevel")}
                        setSelectedOptionVal={setSelectedLevel}
                    ...
                    >
                      ...
                      <SimpleSelect
                        id="department"
                        options={departments}
                        searchLabel={t("employee.addEmployee.searchDepartment")}
                        setSelectedOptionVal={setSelectedDepartment}
