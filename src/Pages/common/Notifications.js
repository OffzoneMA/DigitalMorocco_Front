...
const Notifications = () => {
  ...
  }, [notifications?.notifications, markNotificationsAsRead, refetch]);
          ...
          <div className="flex flex-col items-start justify-start w-full pb-6">
            {notifications?.notifications?.length > 0 ? (
              notifications.notifications.map((item, index) => (
                <div
                  key={index}
                  className="flex flex-col gap-1 py-4  w-full border-b border-gray-201"
                >
                  <p         
                    className={`${
                      item?.read
                        ? "text-[#667084] font-dm-sans-medium"
                        : "text-gray-801 font-dm-sans-bold"
                    } text-sm leading-6`}
                  >
                    {t(item?.message)}
                    {` `}{" "}
                    <span className="text-blue-501 capitalize">
                      {item?.referenceName}
                    </span>
                    {` `}
                    {t(item.message2)}{" "}
              ...
              <div className="flex flex-col items-center h-screen w-full py-28 gap-[32px]">
                ...
                <p
                  ...
                  size=""
                >
                  {t("It looks like you haven't taken any actions yet.")} <br />{" "}
                  {t("Your activity history will appear here, showcasing")}{" "}
                  <br />
                  {t("your interactions and key moments.")}
                </p>
              </div>
            )}
          </div>
