...
export default function ForgotPassword() {
  ...
  const [sendForgotPassword, { isLoading, isSuccess, error }] =
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/ForgotPassword`}
      />
      <div className="bg-gray-100 flex flex-col font-manrope gap-4 mx-auto md:px-10 min-h-screen px-[12px] py-[30px] overflow-y-auto w-full">
        <div
          className={`self-start flex flex-col h-11 items-center justify-center w-auto`}
        >
          <Link
            to="/SignIn"
            className="px-5 items-center self-start flex flex-col h-full w-full cursorpointer-green"
          >
        ...
        <div className="flex flex-col font-dmsans gap-[42px] items-center justify-start mb-[368px]  w-full ">
          ...
          <div className="bg-white-A700 flex flex-col gap-8 items-center justify-start px-5  py-8 rounded-[12px] shadow-formbs max-w-[520px] w-full">
            <Toaster />
            <p className="text-lg font-dm-sans-medium text-gray-901 leading-[32px] sm:text-lg md:text-[22px] w-auto">
              <Text className="text-lg md:text-[22px] font-dm-sans-medium">
            ...
            <div className="flex flex-col gap-8 items-center justify-start w-full">
              <form
                ...
                className="flex flex-col gap-6 items-center justify-start w-full"
              >
                <Text className="text-gray-901 font-dm-sans-medium text-[18px] leading-[28px] w-full">
                  {t("forgot.enterEmailAddress")}
                </Text>
                <div className="flex flex-col gap-4  justify-start w-full">
                  <div className="flex flex-col font-dmsans items-start justify-start w-full">
                    <Text className="text-base font-dm-sans-medium text-[#1D1C21] w-auto">
                  ...
                  <div className="relative w-full">
                    <input
                      ...
                      className={`bg-white w-full border border-solid ${
                        errors?.email || userError
                          ? "border-errorColor shadow-inputBsError "
                          : "border-borderColor"
                      } rounded-full px-[18px] py-[11px] ${
                        errors?.email || userError
                          ? "focus:border-errorColor"
                          : "focus:border-focusColor focus:shadow-inputBs"
                      } placeholder:text-placehColor font-dm-sans-regular placeholder:text-[14px] text-[16px] ${
                        errors?.email || userError
                          ? "!text-errorColor"
                          : "!text-gray-801"
                      }`}
                  ...
                  {(errors?.email?.message || userError || error) && (
                ...
                <div className="flex flex-col gap-4 items-center justify-start w-full">
                  <div
                    className={`${
                      errors?.email
                        ? "bg-gray-202 text-placehColor cursorpointer-green"
                        : isLoading
                        ? "bg-greenbtnhoverbg  text-white-A700"
                        : "bg-teal-A700 text-white-A700 hover:bg-greenbtnhoverbg hover:svg-translate cursorpointer"
                    } my-3 flex flex-row gap-6 h-[52px] md:h-auto items-center justify-center py-[13px] rounded-[26px] w-full `}
                  ...
                  >
                    <button
                      ...
                      className={`text-base items-center ${
                        errors?.email || isLoading ? "disabled" : ""
                      } justify-center font-dm-sans-medium w-auto`}
                    >
                      {isLoading || isLoadingUser
                        ? t("all.sending")
                    ...
                    {isLoading || isLoadingUser ? (
                      ...
                      <svg
                        ...
                        className={`transition-transform transform ${
                          errors?.email ? "text-placehColor" : "text-white-A700"
                        }`}
                      >
                        <path
