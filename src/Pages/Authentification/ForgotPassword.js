...
export default function ForgotPassword() {
  ...
  let [userTrigger, { error: userError, isLoading: isLoadingUser, }] = authApi.endpoints.getUserByEmail.useLazyQuery()
    ...
    try {
      userTrigger(values.email).then(async (payload) => {
        if (payload?.isSuccess) {
          ...
          console.log(payload?.error?.data?.message)
        }
      });
    } catch (error) {
  ...
  return (
    <>
      ...
      />
      <section className="bg-gray-100 flex flex-col font-manrope gap-4 mx-auto md:px-10 min-h-screen px-[12px] py-[30px] overflow-y-auto w-full">
        <nav className={`self-start flex flex-col h-11 items-center justify-center w-auto`}>
          <Link to="/SignIn" className="px-5 items-center self-start flex flex-col h-full w-full cursorpointer">
            <div className="flex flex-row gap-3 items-center justify-center">
              ...
              <span className="text-[#8C8C8E] text-sm tracking-[0.14px]">
                <span className='font-manrope text-sm font-semibold leading-[19.12px] tracking-[0.01em]'>
                  {t('backToLogin')}
                </span>
              </span>
            </div>
        ...
        <section className="flex flex-col font-dmsans gap-[42px] items-center justify-start mb-[368px]  w-full ">
          ...
          <div className="bg-white-A700 flex flex-col gap-8 items-center justify-start px-5  py-8 rounded-[12px] shadow-formbs max-w-[520px] w-full">
            <Toaster />
            <h1 className='text-lg font-dm-sans-medium text-gray-901 leading-[32px] sm:text-lg md:text-[22px] w-auto' >{t('forgot.forgotPassword')} </h1>
