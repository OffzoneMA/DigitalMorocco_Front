...
const ResendVerificationLink = () => {
  ...
  const [userTrigger, { data: userData, error: userError }] =
    authApi.endpoints.getUserByEmail.useLazyQuery();
  const [trigger, { data, status, isSuccess, error: sendError, isLoading }] =
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/ResendVerificationLink`}
      />
      <div
        className={`bg-white-A700 flex flex-col font-DmSans gap-[50px] items-center justify-start mx-auto pb-[50px] w-full min-h-screen overflow-y-auto`}
      >
        ...
        <div className="flex px-3 w-full items-center justify-center">
          <div className="bg-white-A700 shadow-loginModalbsReduced border border-gray-500_33 border-solid flex flex-col gap-6 items-center justify-start max-w-screen-sm p-5 rounded-[14px] w-full">
            <div className="flex flex-col gap-[38px] items-center justify-start max-w-[460px] w-full">
              <img className="w-20 h-20 " src={errorImg} alt="errortick" />
              <div className="flex flex-col gap-1.5 items-center justify-start w-auto ">
                <Text className="leading-[28.00px] font-dm-sans-medium text-gray-801 text-center text-lg ">
                  <span className=" font-dm-sans-medium">
                    {t("verification.linkExpire")}{" "}
                  </span>
                </Text>
                <Text className="flex flex-col mt-3 font-dm-sans-regular leading-[26.00px] text-[#1D2939]  text-center text-sm">
                  <>{t("verification.linkExpireMsg")}</>
                </Text>
                <Text className="flex flex-col font-dm-sans-medium leading-[26.00px] text-[#2575f0]  text-center text-sm">
                  <>{t("verification.linkExpireMsg1")}</>
                </Text>
                <Text className="flex flex-col mt-4 font-dm-sans-regular leading-[26.00px] text-[#1D2939]  text-center text-sm">
                  <>{t("verification.linkExpireMsg2")}</>
                </Text>
                <button
                  className="px-10 py-[18px] mt-6 bg-[#00cdae] rounded-[200px] justify-center items-center gap-6 inline-flex cursorpointer"
            ...
            <Text className="leading-[160.00%] font-dm-sans-regular text-blue_gray-500 text-center text-xs w-full sm:px-16 pt-4">
              <span className="text-blue_gray-500 font-dm-sans-regular">
                {t("chooserole.confirmed.m5")}
              </span>
              <span className="text-blue_gray-500 font-dm-sans-regular"> </span>
