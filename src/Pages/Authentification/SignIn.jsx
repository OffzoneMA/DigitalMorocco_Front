...
export default function SignIn() {
  ...
  const [getUserDetails, { data, isSuccess, isError, detailsError }] =
    authApi.endpoints.getUserDetails.useLazyQuery();
  ...
  useEffect(() => {
    const calculateScale = () => {
      ...
      if (currentHeight <= laptopHeight) {
        ...
        newScale =
          0.95 +
          ((currentHeight - laptopHeight) / (desktopHeight - laptopHeight)) *
            0.05;
    ...
    if (Mount) {
      ...
      if (userInfo) {
        const newLanguage = languages.find(
          (lang) => lang.label === userInfo?.language
        );
          ...
          if (userInfo?.status === "notVerified") {
            ...
            if (!userInfo?.role) {
              ...
              if (userInfo?.role?.toLowerCase() === "admin") {
                ...
                userInfo?.status?.toLowerCase() === "accepted"
              ) {
                navigate("/Dashboard");
              } else if (
                userInfo?.role?.toLowerCase() === "investor" &&
                userInfo?.status?.toLowerCase() === "accepted"
              ) {
                navigate("/Dashboard_Investor");
              } else if (
                userInfo?.role?.toLowerCase() === "partner" &&
                userInfo?.status?.toLowerCase() === "accepted"
              ) {
  ...
  useEffect(() => {
    if (errorSocial) {
      if (errorSocial === "An account already exists with this email") {
        ...
        errorSocial ===
        "This account is not registered using this Social network"
      ) {
        openExistErrorModal();
      } else {
  ...
  useEffect(() => {
    if (error) {
      ...
      if (error === "This email is registered through Google.") {
        ...
        error === "This email is registered through another provider."
      ) {
        setLoginError("Login RS error");
      } else {
        setLoginError("");
  ...
  const handleGoogleButtonClick = () => {
    ...
    }/users/auth/google/signin`;
  };

  const handleLinkedinButtonClick = () => {
    window.location.href = `${
      import.meta.env.VITE_baseURL
    }/users/auth/linkedin/signin`;
  };

  const handleFacebookButtonClick = () => {
    window.location.href = `${
      import.meta.env.VITE_baseURL
    }/users/auth/facebook/signin`;
  };
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/SignIn`}
      />
            ...
            <div className="flex flex-col gap-4 items-center justify-start w-full">
              ...
              <div className="self-stretch flex-col justify-start items-start gap-3 flex w-full">
                ...
                >
                  <div
                    className={`${
                      currentLanguage === "fr" ? "w-[185px] " : "w-[143px]"
                ...
                >
                  <div
                    className={`${
                      currentLanguage === "fr" ? "w-[185px] " : "w-[143px]"
                    } font-dm-sans-medium leading-[normal] text-left text-sm tracking-[0.14px]`}
                  >
              ...
              <div className="flex flex-row gap-2.5 items-center justify-start py-2.5 w-full">
                ...
                <Text className="text-[15px] font-dm-sans-medium  text-gray700 tracking-[0.15px] w-auto">
                  {t("signup.or")}
                </Text>
                  ...
                  <div className="flex flex-col gap-4 items-center justify-start w-full">
                    <div className="flex flex-col gap-2 items-start justify-start w-full">
                      <input
                        ...
                        className={`bg-white-A700 transition-all duration-200 w-full border border-solid ${
                          errors?.email ||
                          (getValues("email")?.length > 0 &&
                            (error == "Wrong email ." ||
                              loginError == "Login RS error"))
                        ...
                        } placeholder:text-placehColor  placeholder:text-[14px] text-[15px] text-${
                          errors?.email ||
                          (getValues("email")?.length > 0 &&
                            (error == "Wrong email ." ||
                              loginError == "Login RS error"))
                    ...
                    <div className="flex flex-col gap-2 items-start justify-start w-full">
                      <div className="relative w-full">
                        <input
                          ...
                          } transition-all duration-200 placeholder:tracking-normal bg-white w-full  border border-solid ${
                            errors?.password ||
                    ...
                    <div className="flex flex-row gap-2.5 items-center justify-between  w-full">
                      <div className="flex flex-row flex-1 items-center justify-start m-auto w-auto">
                        <label
                          htmlFor={`rememberme`}
                          className="cursorpointer relative inline-flex items-center justify-center peer-checked:border-0 rounded-[3px] mr-2"
                        >
                          <input
                            {...register("rememberme", {
                              required: {
                                value: false,
                                message: t("signup.termsValidation"),
                              },
                            })}
                            id={`rememberme`}
                            type="checkbox"
                        ...
                        <label
                          htmlFor={`rememberme`}
                          className="text-[13px] mt-[0.5px] text-center font-dm-sans-regular leading-[16.93px] tracking-[0.01em] text-gray700 w-auto cursorpointer"
                        >
                          {t("signin.rememberMe")}
                        </label>
                      </div>
                    ...
                    <div className="flex flex-col items-center justify-start w-full mt-[5px] ">
                      <button
                        ...
                        className={`
                                ...
                                ${
                                  loading
                                    ? "bg-greenbtnhoverbg cursor-not-allowed"
                                    : "bg-teal-A700 hover:bg-greenbtnhoverbg active:bg-teal-700 hover:svg-translate "
                                }
                              `}
                        disabled={loading}
                      >
                      ...
                      <div className="flex flex-row gap-2.5 items-center justify-start w-auto mt-5">
                        <Text className="font-dm-sans-medium text-[#37363B] text-sm w-auto">
      ...
      <EmailExistModalOrConfirmation
        isOpen={isExistErrorModalOpen}
        onRequestClose={closeExistErrorModal}
        content={
