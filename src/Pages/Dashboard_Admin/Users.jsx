...
const Users = () => {
  ...
  if (filterApply) {
    ...
    queryParams.date = selectedDate?.trim()
      ? parseDateStringValue(selectedDate)
      : null;
  ...
  const {
    ...
    isFetching: loading,
    refetch,
  } = useGetAllUsersPageQuery(queryParams);
  ...
  const pageData = users?.filter((item) => {
    const keywordMatch = item?.displayName
      ?.toLowerCase()
  ...
  //     const response = await axios.get(`${import.meta.env.VITE_baseURL}/users/AllUsers`, {
  //       headers: {
    ...
    if (confirmation.isConfirmed) {
      try {
        ...
        const response = await axios.put(
          ...
          }/users/ApproveUser/${userId}?role=${role}`,
          {},
          {
            headers: {
    ...
    if (confirmation.isConfirmed) {
      try {
        ...
        await axios.put(
          `${
            import.meta.env.VITE_baseURL
          }/users/RejectUser/${userId}?role=${role}`,
          {},
          {
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <>
      <HelmetWrapper
        ...
        description={t("helmet.users.description")}
        keywords={t("helmet.users.keywords")}
        canonical={`${import.meta.env.VITE_URL}/Users`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("sidebar.users")}</PageHeader>
            </div>
          ...
          <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
            <div className="w-full bg-white-A700 border border-gray-201 rounded-[8px] shadow-tablesbs  ">
              <div className="flex flex-row flex-wrap gap-5 items-center border-b border-gray-201 rounded-t-lg bg-white-A700  py-[19.5px] px-5">
                <TableTitle>{t("users.unapprovedUsers")}</TableTitle>
                  ...
                  {filter && (
                    <>
                      ...
                      <SimpleSelect
                        ...
                        setSelectedOptionVal={setSelectedSortField}
                      ...
                      <SimpleSelect
                        ...
                        id="order"
                        options={["Asc", "Desc"]}
                        setSelectedOptionVal={setSelectedSortOrder}
                      ...
                      {selectedDate && selectedDate?.trim() !== "" && (
                        <SimpleSelect
                          className="min-w-[170px]"
                          id="dateField"
                          options={["Date Created", "Last Login"]}
                          setSelectedOptionVal={setSelectedDateField}
                      ...
                      <MultipleSelect
                        ...
                        searchLabel={t("common.searchRole")}
                        setSelectedOptionVal={setSelectedRoles}
                        placeholder={t("common.selectRole")}
                        content={(option) => {
                          return (
                            <div className="flex  py-2 items-center  w-full">
                              <Text className="text-gray-801 text-left text-base font-dm-sans-regular leading-5 w-auto capitalize">
                                {t(`${option}`)}
                              </Text>
                            </div>
                          );
                      ...
                      <MultipleSelect
                        ...
                        searchLabel={t("common.searchStatus")}
                        setSelectedOptionVal={setSelectedStatuses}
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[44px] ">
                      <th className="p-3 text-left text-blue_gray-800_01 font-medium ">
                        {t("users.displayName")}
                      </th>
                      <th className="p-3 text-left text-blue_gray-800_01 font-medium ">
                        {t("users.googleId")}
                        ...
                        >
                          ...
                          <td className="px-[18px] py-4 text-gray-900_01">
                            <div className="flex flex-row space-x-3 items-center">
                              ...
                              >
                                {renderTableCell(user.linkedinId)}
                              </span>
                            </div>
                          </td>
                          <td className="px-[18px] py-4 text-gray-900_01">
                          ...
                          <td className="px-[18px] py-4 text-gray-900_01">
                            <div
                              className={`inline-flex px-3 h-[34px] justify-center items-center capitalize ${
                          ...
                          <td
                            className="py-4 px-3 text-gray500"
                            style={{
                              whiteSpace: "nowrap",
                              overflow: "hidden",
                              textOverflow: "ellipsis",
                          ...
                          <td className="px-[18px] py-4 text-gray-900_01">
                            <div className="flex flex-row space-x-3 items-center">
                              ...
                              >
                                {renderTableCell(user.lastLogin)}
                              </span>
                            </div>
                          </td>
                          <td className="px-[18px] py-4 text-gray-900_01">
                          ...
                          <td className="px-[18px] py-4 ">
                            {user?.status === "pending" && (
                              <div className="flex flex-row space-x-3 px-3 items-center">
                                <div
                                  className="relative group w-[38px] h-8 px-3 py-2 bg-[#aeb6c5] hover:bg-[#00CDAE] rounded-md justify-center items-center gap-3 inline-flex "
                                  onClick={() =>
                                    handleApproveUser(user._id, user.role)
                                  }
                                >
                                ...
                                <div
                                  className="relative group w-[38px] h-8 px-3 py-2 bg-[#aeb6c5] hover:bg-[#EF4352] rounded-md justify-center items-center gap-3 inline-flex "
                                  onClick={() =>
                                    handleRejectUser(user._id, user.role)
                                  }
                                >
                                  <div className="justify-center items-center gap-2 flex">
                                    <HiBan
