...
const Dashboard_Admin = () => {
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    monthsOrder1[currentMonthIndex]
  );
  ...
  useEffect(() => {
    if (users) {
      ...
      const investors = filteredUsers.filter(
  ...
  const groupUsersByMonth = (users, year) => {
    const months = [
      t("common.shortMonths.january"),
      t("common.shortMonths.february"),
      t("common.shortMonths.march"),
      t("common.shortMonths.april"),
      t("common.shortMonths.may"),
      t("common.shortMonths.june"),
      t("common.shortMonths.july"),
      t("common.shortMonths.august"),
      t("common.shortMonths.september"),
      t("common.shortMonths.october"),
      t("common.shortMonths.november"),
      t("common.shortMonths.december"),
    ];
    const grouped = {};
  ...
  const groupUsersByWeek = (users, year, month) => {
    ...
    users.forEach((user) => {
      ...
      if (
        ...
        date.getMonth() === monthsOrder1.indexOf(month)
      ) {
        const week = `${t("adminDashboard.week")} ${Math.ceil(
          (date.getDate() +
  ...
  useEffect(() => {
    if (users) {
      ...
      const groupedData =
        timeFrame === "month"
          ? groupUsersByMonth(roleUsers, currentYear)
          : groupUsersByWeek(roleUsers, currentYear, selectedMonth);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip flex flex-row bg-transparent">
          <p className="label" onClick={() => setSelectedMonth(name)}>
            {name}: <span className="text-[#45C68A] ">{value}</span>
          </p>
        </div>
      );
    }
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/Dashboard_Admin`}
      />
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen overflow-auto items-start justify-start pb-14 pt-8 rounded-tl-[40px] w-full">
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          ...
          <div className="flex pb-6">
            <Text className="text-lg font-inter text-gray-500 leading-6 tracking-normal w-full">
              {t("adminDashboard.trackManageForecast")}
            </Text>
          </div>
          <div className="flex flex-wrap justify-center gap-10 pt-8 w-full">
            <div
              className={`flex flex-col basis-[230px] grow max-w-[600px] gap-3 items-center rounded-[12px] border animation py-7 px-5 hover:border-blue-503 hover:shadow-roleCardbs ${
                role === "member"
              ...
              <div className="flex flex-row items-center w-full justify-between">
                <div className="flex flex-row items-center w-full">
                  ...
                  <div className="flex flex-col p-3 items-center gap-1 ml-2">
                    <Text className=" ext-base font-dm-sans-medium leading-8 text-gray-900_01 tracking-normal w-full">
                      {t("adminDashboard.signUpVolume")}
                    </Text>
                    <Text className="text-sm font-dm-sans-regular leading-[26px] text-blue_gray-301 tracking-normal  w-full">
                ...
                <div className="flex flex-row items-center justify-center border border-gray_200 px-[9px] py-[7px] rounded-[48px] w-auto gap-2.5">
                  <div
                    className="h-[19px] text-[13px] text-[#303030] font-Montserrat-semiBold "
                    style={{ whiteSpace: "nowrap" }}
                  >
                    {t("adminDashboard.showBy")}
                  </div>
                  <div className="w-auto justify-start items-center gap-2.5 flex">
                    <div
                      className="px-3 py-[3px] bg-[#AAAAAA1A] rounded-[100px] justify-center items-center gap-2.5 flex"
                      ...
                      <div
                        className={`${
                          timeFrame === "week"
                            ? "text-purple-500"
                            : "text-[#303030]"
                        } text-sm font-medium font-dm-sans-regular`}
                      >
                      ...
                      <div
                        className={`${
                          timeFrame === "month"
                            ? "text-purple-500"
                            : "text-[#303030]"
                        } text-sm font-normal font-dm-sans-regular`}
                      >
                        {t("adminDashboard.month")}
                      </div>
                    </div>
                    <div
                      className={`px-3 py-[3px] bg-[#AAAAAA1A] rounded-[100px] justify-center items-center gap-2.5 flex relative`}
                      onMouseEnter={() => setIsOpen(true)}
                      onMouseLeave={() => setIsOpen(false)}
                      onClick={() => setIsOpen(!isOpen)}
                    >
                      <div
                        className="text-[#303030] text-sm font-normal font-dm-sans-regular"
                        style={{ whiteSpace: "nowrap" }}
                      >
                        {selectedMonth || t("adminDashboard.selectMonth")}
                      </div>
                      {isOpen && (
