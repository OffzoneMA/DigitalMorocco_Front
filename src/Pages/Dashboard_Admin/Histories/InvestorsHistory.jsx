...
const InvestorsHistory = () => {
  ...
  const { data: users, isLoading: isLoadingUsers } =
    useGetHistoryUsersQuery("investor");
  ...
  const { data, error, isLoading, refetch } =
    useGetInvestorActivityHistoriesQuery(queryParams);
  ...
  useEffect(() => {
    if (filterApply) {
      const isAllFiltersEmpty =
        localSelectedUsers?.length === 0 && !localSelectedDate?.trim();
  ...
  return (
    <>
      <HelmetWrapper
        title={t("helmet.adminHistory.investors.title")}
        description={t("helmet.adminHistory.investors.description")}
        keywords={t("helmet.adminHistory.investors.keywords")}
        canonical={`${import.meta.env.VITE_URL}/InvestorsHistory`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("sidebar.adminHistory.main")}</PageHeader>
            </div>
                ...
                {filter && (
                  <>
                    ...
                    <MultipleSelect
                      ...
                      setSelectedOptionVal={setLocalSelectedUsers}
