...
const Users = () => {
  ...
  useEffect(() => {
    ...
    setCur(data?.pagination?.currentPage || 1);
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${process.env.REACT_APP_URL}/Users`}
      />
                    ...
                    <tbody className="font-dm-sans-regular text-sm leading-6">
                      {
                        pageData.map((user, index) => (
                          <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-50' : ''} transition-all duration-300 ease-in-out`}>
