    ...
    <>
      ...
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen overflow-auto items-start justify-start pb-14 pt-8 rounded-tl-[40px] w-full">
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 pb-8 w-full">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 md:gap-6 pt-6 w-full">
                      ...
                      <div className="flex flex-row gap-2 py-2 w-full">
                        <div className="flex flex-col flex-1 items-start justify-start">
                          ...
                          <div className="flex flex-col items-start justify-center py-4 w-full">
                            ...
                            >
                              {item?.currency} {item.funding?.toLocaleString('fr-FR').replace(/\s/g, '\u00A0') || 0}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col flex-1 items-start justify-start">
                          ...
                          <div className="flex flex-col items-start justify-start py-4 w-full">
                            <p
                              className="text-[22px] font-dm-sans-medium text-[#344054] sm:text-lg w-auto"
                            >
                              {item?.currency} {item?.totalRaised?.toLocaleString('fr-FR').replace(/\s/g, '\u00A0') || 0}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      ? Requestdata.map((item, index) => (
                        <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-50' : ''} hover:bg-blue-50 cursorpointer w-full`} onClick={() => navigate(`/InvestorDetails/${item?.investor?._id}`)}>
                          ...
                          <td className="py-4 px-3 text-gray-600 text-sm font-dm-sans-regular leading-6">
                            <div
                              className={`flex flex-row whitespace-nowrap space-x-2 items-center py-1 px-2 text-sm font-dm-sans-regular leading-6 rounded-full ${(item.status === 'Approved' || item.status === 'Accepted')
