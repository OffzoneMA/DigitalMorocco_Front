...
const CreateProject = () => {
  ...
  const {
    ...
    setValue,
    trigger,
    formState: { errors, isSubmitting, isValid: validForm },
  } = useForm({
    defaultValues: project
      ? {
          ...
          totalRaised: project?.totalRaised
            ? formatNumber(project.totalRaised)
            : "",
  ...
  useEffect(() => {
    ...
    const isValid =
      isCountryValid && isStageValid && isSectorValid && isPublicationValid;
  ...
  useEffect(() => {
    if (
      ...
      (loadingDel !== null && fetchedProject)
    ) {
  ...
  const fetchMembers = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_baseURL}/employee/byuserWithoutPage`,
        {
          headers: {
  ...
  useEffect(() => {
    if (project) {
      ...
      const initialFormattedMilestones =
      ...
      const otherDocuments = project.documents?.filter(
      ...
      setSelectedStatus(project?.status || "");
      if (project?.country) {
        const defaultCountry = dataCountries.find(
          (country) => country.name === project.country
        );
  ...
  const formatFundingValue = (value) => {
    ...
    setValue("funding", formattedValue, {
      shouldValidate: true,
      shouldDirty: true,
  ...
  const removeMilestone = async (id) => {
    const milestone = milestones.find(
      (milestone) => milestone.id === id || milestone._id === id
    );
  ...
  const handleFileRemove = async (type) => {
    ...
    const documentToRemove = project?.documents.find(
    ...
    if (documentToRemove && documentToRemove._id && projectId) {
      try {
        ...
        refetch();
        console.log(
          `Document avec ID ${documentToRemove._id} supprimé de la base de données.`
        );
      } catch (error) {
        console.error(
          "Erreur lors de la suppression du document dans la base de données :",
          error
        );
        return;
      }
  ...
  const handleStatusChangeAndUpdate = async () => {
    ...
    if (
      ...
      selectedStatus?.toLowerCase() === "stand by"
    ) {
      newStatus = "Active";
    ...
    if (projectId) {
      try {
        ...
        const response = await axios.patch(
          `${import.meta.env.VITE_baseURL}/projects/${projectId}/status`,
          {
            status: newStatus,
          }
        );
  ...
  return (
    <>
      <HelmetWrapper
        title={
          !projectId
            ? t("helmet.projects.create.title")
            : t("helmet.projects.edit.title")
        }
        description={
          !projectId
            ? t("helmet.projects.create.description")
            : t("helmet.projects.edit.description")
        }
        keywords={
          !projectId
            ? t("helmet.projects.create.keywords")
            : t("helmet.projects.edit.keywords")
        }
        canonical={
          !projectId
            ? `${import.meta.env.VITE_URL}/Createproject`
            : `${import.meta.env.VITE_URL}/Editproject/${projectId}`
        }
      />
      <section
        id="create-edit-project"
        className="bg-white-A700 flex flex-col gap-8 items-start justify-start pb-8 pt-8 rounded-tl-[40px] h-full min-h-screen overflow-auto w-full"
      >
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full pb-6">
          <form
            ...
            className="w-full h-full bg-white-A700 border border-gray-201 rounded-[8px] pb-3 shadow-tablesbs "
          >
            <div className="flex flex-row flex-wrap text-sm text-center text-gray-500 border-b border-gray-201 rounded-t-lg bg-white-A700 h-[77px] py-[19px] px-5">
              ...
              <div className="flex flex-row ml-auto gap-[16px] items-center">
                      ...
                      <Popup
                        ...
                        trigger={(open) => (
                          <button
                            ...
                            className={`${
                              isAllFormValid && validForm
                                ? "bg-teal-A700 text-white-A700 hover:bg-greenbtnhoverbg active:bg-[#018080]"
                                : "bg-[#e5e5e6] text-[#a7a6a8]"
                            } text-sm font-dm-sans-medium flex flex-row h-[37px] min-w-[85px] gap-[8px] items-center justify-center px-[12px] cursorpointer rounded-md`}
                            type="button"
                            disabled={!(isAllFormValid && validForm)}
                          >
                            <svg
                              width="21"
                              height="21"
                        ...
                        <div className="w-[259px] h-[59px] px-[18px] py-[10px] bg-[#2C3462] rounded-lg justify-center items-center flex">
                          <div className="grow w-full shrink basis-0 h-full justify-center items-center flex">
                            <div className="w-full text-[#D0D5DD] text-[10px] font-dm-sans-regular">
                              {t(
                                "projects.createNewProject.activatePop.description"
                              )}{" "}
                              <span className="font-dm-sans-medium text-[#35D8BF] ">
                                {t(
                                  "projects.createNewProject.activatePop.title"
                                )}
                              </span>
                              ,{" "}
                              {t(
                                "projects.createNewProject.activatePop.activatePop"
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      >
                        <div className="w-[259px] h-[59px] px-[18px] py-[10px] bg-[#2C3462] rounded-lg justify-center items-center flex">
                          <div className="grow w-full shrink basis-0 h-full justify-center items-center flex">
                            <div className="w-full text-[#D0D5DD] text-[10px] font-dm-sans-regular">
                              {t(
                                "projects.createNewProject.standByPop.description"
                              )}{" "}
                              <span className="font-dm-sans-medium text-[#35D8BF] ">
                                {t(
                                  "projects.createNewProject.standByPop.title"
                                )}
                              </span>
                              ,{" "}
                              {t(
                                "projects.createNewProject.standByPop.standByPop"
                              )}
                            </div>
                          </div>
                        </div>
                ...
                <button
                  ...
                  className={`${
                    submitting === "ok"
                      ? "bg-teal-A700 !cursor-not-allowed"
                      : !submitting === "sending"
                      ? "bg-blue-A400 !cursor-not-allowed"
                      : "bg-blue-A400  hover:bg-[#235DBD] active:bg-[#224a94]"
                  } text-sm font-dm-sans-medium text-white-A700 flex flex-row h-[37px] min-w-[85px] items-center justify-center px-[12px] cursorpointer rounded-md`}
                  ref={formButtonRef}
            ...
            <div className="flex flex-col flex-wrap md:flex-row lg:flex-row xl:flex-row 2xl:flex-row gap-8 items-start justify-start px-6 pt-5 pb-9 bg-white-A700 w-full h-auto">
              <div
                ref={div1Ref}
                className="flex md:min-w-[300px] flex-1 flex-col gap-6 items-start justify-start w-full h-full"
              >
                <div
                ...
                >
                  ...
                  <input
                    ...
                    className={`!placeholder:text-blue_gray-300 !text-gray700 leading-[18.2px] font-manrope text-left text-sm tracking-[0.14px] w-full rounded-[6px] px-[12px] py-[10px] h-[40px] border border-[#D0D5DD] ${
                      errors?.name
                        ? "border-errorColor shadow-inputBsError focus:border-errorColor"
                        : "border-[#D0D5DD] focus:border-focusColor focus:shadow-inputBs"
                    }`}
                    type="text"
                    name="name"
                    placeholder={t(
                      "projects.createNewProject.enterProjectName"
                    )}
                  />
                ...
                >
                  ...
                  <textarea
                    ...
                    className={`!placeholder:text-blue_gray-300 !text-gray700 max-h-[139px] leading-[18.2px] font-manrope text-left text-sm tracking-[0.14px] w-full rounded-[6px] px-[12px] py-[10px]  border border-[#D0D5DD] ${
                      errors?.details
                        ? "border-errorColor shadow-inputBsError focus:border-errorColor"
                        : "border-[#D0D5DD] focus:border-focusColor focus:shadow-inputBs"
                    }`}
                    ...
                    rows={7}
                    placeholder={t(
                      "projects.createNewProject.enterProjectDetails"
                    )}
                    style={{
                      scrollbarWidth: "none",
                ...
                >
                  ...
                  <input
                    {...register("contactEmail", {
                      required: {
                        value: true,
                        message: "Project Contact Email is required",
                      },
                      minLength: {
                        value: 2,
                      },
                ...
                >
                  ...
                  <MultipleSelect
                    id="teams"
                    options={members}
                    searchLabel={t("projects.createNewProject.searchMember")}
                    setSelectedOptionVal={setSelectedTeamsMember}
                    selectedOptionsDfault={selectedProjectTeamsMembers}
                    itemClassName="py-2 border-b border-gray-201"
                    placeholder={t(
                      "projects.createNewProject.assignTeamMember"
                    )}
                    valuekey="fullName"
                    optionkey="workEmail"
                    emptyMsg={t("employee.noTeamMembers")}
                    emptyIcon={<PiUsersThin size={30} />}
                    content={(option) => {
                      return (
                        <div className="flex items-center  space-x-3 ">
                          <img
                            src={option?.image || userdefaultProfile}
                            alt="teams"
                  ...
                  <div className="relative flex items-center w-full">
                    ...
                    <input
                      {...register("funding", {
                        required: {
                          value: true,
                          message: "Project Funding Target is required",
                        },
                        validate: {
                          hasValue: (value) =>
                            cleanNumber(value)?.length > 0 ||
                            "Project Funding Target is required",
                        },
                      })}
                      className={`!placeholder:text-blue_gray-300 !text-gray700 leading-[18.2px] font-manrope text-left text-sm tracking-[0.14px] w-full rounded-[6px] px-[28px] py-[10px] h-[40px] border ${
                        errors?.funding
                          ? "border-errorColor shadow-inputBsError focus:border-errorColor"
                          : "border-[#D0D5DD] focus:border-focusColor focus:shadow-inputBs"
                      }`}
                      ...
                      placeholder={t(
                        "projects.createNewProject.enterFundingTarget"
                      )}
                    />
                  </div>
                ...
                <div className="flex flex-col gap-2 items-start justify-start w-full">
                  ...
                  <div className="relative flex items-center w-full">
                    ...
                    <input
                      ...
                      placeholder={t(
                        "projects.createNewProject.enterTotalRaised"
                      )}
                    />
                  </div>
                ...
                >
                  ...
                  <SimpleSelect
                    id="stage"
                    options={stagesData}
                    searchLabel={t("common.searchStage")}
                    setSelectedOptionVal={setSelectedStage}
                    placeholder={t("projects.createNewProject.selectStage")}
                    selectedOptionsDfault={project?.stage || ""}
                    required={requiredFields.stage}
                    sortable={false}
                    content={(option) => {
                      return (
                ...
                >
                  ...
                  <SimpleSelect
                    ...
                    placeholder={t("projects.createNewProject.selectCountry")}
                    valuekey="name"
                ...
                >
                  ...
                  <SimpleSelect
                    id="sector"
                    options={companyType}
                    searchLabel={t("common.searchSector")}
                    searchable={true}
                    setSelectedOptionVal={setselectedSector}
                    placeholder={t(
                      "projects.createNewProject.selectProjectSector"
                    )}
                    selectedOptionsDfault={project?.sector || ""}
                ...
                >
                  ...
                  <Text className="text-[12px] font-dm-sans-regular text-[#98A2B3] w-auto">
                    {t(
                      "projects.createNewProject.projectPublication.description"
                    )}
                  </Text>
                  <SimpleSelect
                    id="publication"
                    options={["Public", "Private"]}
                    selectedOptionsDfault={project?.visbility}
                    setSelectedOptionVal={setSelectedPublication}
                    searchable={false}
                    placeholder={t(
                      "projects.createNewProject.projectPublication.selectType"
                    )}
                    required={requiredFields.publication}
                    content={(option) => {
                      return (
                        <div className="flex  py-2 items-center  w-full">
                          <Text className="text-gray-801 text-left text-base capitalize font-dm-sans-regular leading-5 w-auto">
                            {t(`${option}`)}
                          </Text>
                        </div>
                      );
                ...
                >
                    ...
                    {milestones?.length > 1 && (
                      ...
                      >
                        ...
                        <span className="hidden md:inline">
                          {t("projects.createNewProject.more")}
                        </span>
                      </button>
                    )}
                  </div>
                  {milestones.map((milestone, index) => (
                    <div
                      key={milestone.id}
                      className={`flex flex-row gap-2 items-start justify-start w-full`}
                    >
                      <div className="flex md:flex-1 w-[55%]">
                        <input
                          ...
                          placeholder={t(
                            "projects.createNewProject.enterProjectMilestone"
                          )}
                      ...
                      <CustomCalendar
                        className={" w-[30%]"}
                        inputPlaceholder={t(
                          "projects.createNewProject.dueDate"
                        )}
                      ...
                      {milestones?.length === 1 ? (
                        ...
                        >
                          {loadingDel !== null &&
                          loadingDel === milestone?.id ? (
                            <AiOutlineLoading
                              size={22}
                              className="animate-spin disabled !cursor-not-allowed"
                            />
                          ) : (
              ...
              <div
                ref={dividerRef}
                className={`bg-gray-201 lg:min-h-fit lg:h-[${maxDivHeight}] lg:max-h-[${maxDivHeight}] h-px w-full lg:w-px`}
              />
              ...
              <div
                ref={div2Ref}
                className="flex flex-col gap-6 items-start justify-start lg:w-[40%] w-full"
              >
                <div
                  className={`flex flex-col gap-2 items-start justify-start w-full`}
                >
                  ...
                  <div
                    className="bg-white-A700 border border-blue_gray-100_01 border-solid h-[270px] flex flex-col items-center justify-center rounded-md w-full py-1 cursorpointer relative"
                    ...
                    {logoFile ? (
                      <>
                        <img
                          src={logoFile}
                          alt="Uploaded Logo"
                          className="rounded-md w-full h-[268px]"
                        />
                        <div
                          className="absolute top-2 right-0 flex flex-col justify-end"
                        ...
                        >
                          <div className="relative mr-3 w-auto">
                            ...
                            >
                              <path
                                d="M7.0013 2.66659C7.36949 2.66659 7.66797 2.36811 7.66797 1.99992C7.66797 1.63173 7.36949 1.33325 7.0013 1.33325C6.63311 1.33325 6.33464 1.63173 6.33464 1.99992C6.33464 2.36811 6.63311 2.66659 7.0013 2.66659Z"
                                stroke="#1D2939"
                                strokeWidth="1.4"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                              />
                              <path
                                d="M11.668 2.66659C12.0362 2.66659 12.3346 2.36811 12.3346 1.99992C12.3346 1.63173 12.0362 1.33325 11.668 1.33325C11.2998 1.33325 11.0013 1.63173 11.0013 1.99992C11.0013 2.36811 11.2998 2.66659 11.668 2.66659Z"
                                stroke="#1D2939"
                                strokeWidth="1.4"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                              />
                              <path
                                d="M2.33464 2.66659C2.70283 2.66659 3.0013 2.36811 3.0013 1.99992C3.0013 1.63173 2.70283 1.33325 2.33464 1.33325C1.96645 1.33325 1.66797 1.63173 1.66797 1.99992C1.66797 2.36811 1.96645 2.66659 2.33464 2.66659Z"
                                stroke="#1D2939"
                                strokeWidth="1.4"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                          ...
                          {showLogoDropdown && (
                            <div className="absolute top-[100%] right-0 flex flex-col">
                              <div className="flex mt-1 flex-col bg-white-A700 border-[0.5px] border-[#2575F01A] rounded-[8px] p-[16px] shadow-roleCardbs z-10">
                                <div
                                  className="w-auto group h-9 py-[5px] px-[16px] justify-start items-center gap-3 inline-flex"
                                ...
                                >
                                  ...
                                  <div className="text-[#1d2838] group-hover:text-[#2575F0] transition-colors duration-300">
                                    {t("common.change")}
                                  </div>
                                </div>
                                <div
                                ...
                                >
                                  <span>
                                    ...
                                    >
                                      <path
                                        d="M5 1.5H9M1 3.5H13M11.6667 3.5L11.1991 10.5129C11.129 11.565 11.0939 12.0911 10.8667 12.49C10.6666 12.8412 10.3648 13.1235 10.0011 13.2998C9.58798 13.5 9.06073 13.5 8.00623 13.5H5.99377C4.93927 13.5 4.41202 13.5 3.99889 13.2998C3.63517 13.1235 3.33339 12.8412 3.13332 12.49C2.90607 12.0911 2.871 11.565 2.80086 10.5129L2.33333 3.5M5.66667 6.5V9.83333M8.33333 6.5V9.83333"
                                        stroke="#2575F0"
                                        strokeWidth="1.4"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                          ...
                          <div className="flex flex-col items-center justify-center w-auto">
                            ...
                            >
                              {isDragging
                                ? t("common.dropLogo")
                                : t(
                                    "projects.createNewProject.projectLogo.uploadLogo"
                                  )}
                            </Text>
                          </div>
                        </div>
                ...
                >
                  ...
                  <div
                    className="flex md:flex-1 w-full md:w-full rounded-md px-2 py-3 border border-dashed cursorpointer bg-blue_gray-50"
                    ...
                    <label className="font-manrope font-normal text-sm leading-18 tracking-wide text-left w-auto">
                      {isDragging ? (
                        <span className="text-blue_gray-300">
                          {t(
                            "projects.createNewProject.uploadPitchDeck.dropHere"
                          )}
                        </span>
                      ) : (
                        <>
                          <span className="text-blue_gray-300">
                            {" "}
                            {t(
                              "projects.createNewProject.uploadPitchDeck.description"
                            )}{" "}
                          </span>
                          <span className="text-blue-500">
                            {t(
                              "projects.createNewProject.uploadPitchDeck.description1"
                            )}
                          </span>
                        </>
                      )}
                        ...
                        >
                          {fileNames["pitchDeck"] ||
                            project?.documents?.filter(
                              (document) =>
                                document.documentType === "pitchDeck"
                            ).name}
                        </Text>
                      </div>
                      <div
                        className="flex w-auto"
                        onClick={() => handleFileRemove("pitchDeck")}
                      >
                        <svg
                          width="16"
                          height="16"
                ...
                >
                  ...
                  <div
                    className="flex md:flex-1 w-full md:w-full rounded-md px-2 py-3 cursorpointer border border-dashed bg-blue_gray-50"
                  ...
                  >
                    ...
                    <label className="font-manrope font-normal text-sm leading-18 tracking-wide text-left w-auto">
                      {isDragging ? (
                        <span className="text-blue_gray-300">
                          {t(
                            "projects.createNewProject.uploadBusinessPlan.dropHere"
                          )}
                        </span>
                      ) : (
                        <>
                  ...
                  {fileNames["businessPlan"] && (
                    <div className="flex flex-row gap-2 items-center justify-between w-[90%] pt-2 ">
                      ...
                      <div
                        className="flex w-auto"
                        onClick={() => handleFileRemove("businessPlan")}
                      >
                        <svg
                          width="16"
                          height="16"
                ...
                <div
                  className={`flex flex-col gap-2 items-start cursorpointer justify-start w-full`}
                  onDragOver={handleDragOver}
                ...
                >
                  ...
                  >
                    {t(
                      "projects.createNewProject.uploadFinancialProjection.title"
                    )}
                  </Text>
                  <div
                    className="flex md:flex-1 w-full md:w-full rounded-md px-2 py-3 border border-dashed bg-blue_gray-50"
                  ...
                  >
                    ...
                    <input
                      ref={inputRef2}
                      onChange={(e) =>
                        handleFileUpload1(e, "financialProjection")
                      }
                      style={{ display: "none" }}
                      className={`!placeholder:text-blue_gray-300 !text-gray700 font-manrope p-0 text-left text-sm tracking-[0.14px] w-full bg-transparent border-0`}
                      type="file"
                    ...
                    <label className="font-manrope font-normal text-sm leading-18 tracking-wide text-left w-auto">
                      {isDragging ? (
                        <span className="text-blue_gray-300">
                          {t(
                            "projects.createNewProject.uploadFinancialProjection.dropHere"
                          )}
                        </span>
                      ) : (
                        <>
                  ...
                  {fileNames["financialProjection"] && (
                    <div className="flex flex-row gap-2 items-center justify-between w-[90%] pt-2">
                      ...
                      <div
                        className="flex w-auto"
                        onClick={() => handleFileRemove("financialProjection")}
                      >
                        <svg
                          width="16"
                          height="16"
                    ...
                    {Array.from(droppedFiles.values()).map((file) => {
                      if (file?.index === index) {
                        return (
                          <div
                            key={`file-${file.index}`}
                            className="flex flex-row gap-2 items-center justify-between w-[90%] pt-2"
                          >
                            <div className="flex flex-row gap-2 items-center justify-start w-full">
                              <GrAttachment size={16} className="mr-2" />
                              <Text
