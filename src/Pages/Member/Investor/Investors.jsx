...
import {
  ...
  useGetInvestorsListForMemberQuery,
...
const Investors = () => {
  ...
  if (filterApply) {
    queryParams.type =
      investmentType.length > 0 ? investmentType.join(",") : undefined;
    queryParams.location = location || undefined;
    queryParams.industries =
      industries.length > 0 ? industries.join(",") : undefined;
  ...
  const {
    ...
    isFetching: loading,
    refetch,
  } = useGetInvestorsListForMemberQuery(queryParams);
  const { data: locationData, isLoading: locationLoading } =
    useGetDistinctValuesQuery("location");
  const { data: industryData, isLoading: industryLoading } =
    useGetDistinctValuesQuery("PreferredInvestmentIndustry");
  const { data: typeData, isLoading: typeLoading } =
    useGetDistinctValuesQuery("type");
    ...
    filterApply
      ? data?.filter((item) => {
          ...
          if (Array.isArray(item?.PreferredInvestmentIndustry)) {
            ...
            const industriesMatch = item.PreferredInvestmentIndustry.some(
              (industry) => {
          ...
          if (item?.numberOfInvestment && !isNaN(Number(normalizedKeyword))) {
            if (item.numberOfInvestment === Number(normalizedKeyword))
              return true;
          }
  ...
  const handleInvestorClick = (investor) => {
    ...
    navigate(`/InvestorDetails/${investor?._id}`, {
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/Investors`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("sidebar.investor.main")}</PageHeader>
            </div>
            ...
            <div className="w-full bg-white-A700 border border-gray-201 rounded-[8px] shadow-tablesbs  ">
                ...
                <div className="md:flex md:flex-1 md:flex-wrap md:flex-row grid grid-cols-2 grid-flow-row auto-cols-min gap-3 w-auto items-center md:justify-end md:ml-auto w-auto">
                  {filter && (
                    <>
                      ...
                      <MultipleSelect
                        className="min-w-[170px] max-w-[250px]"
                        id="investor"
                        options={typeData}
                        searchLabel={t("common.searchType")}
                        setSelectedOptionVal={setLocalInvestmentType}
                      ...
                      />
                      <MultipleSelect
                        className="min-w-[170px] max-w-[300px]"
                        id="investor"
                        options={industryData}
                        searchLabel={t("common.searchIndustry")}
                        setSelectedOptionVal={setLocalIndustries}
              ...
              <div className="relative flex flex-col w-full">
                  ...
                  <table className=" w-full">
                    <thead>
                      <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[44px]">
                        ...
                        <th
                          scope="col"
                          className="px-[18px] py-3 text-center text-[#344054] font-DmSans font-medium"
                        >
                          {t("investors.numberOfInvestments")}
                        </th>
                        <th
                          scope="col"
                          className="px-[18px] py-3 text-center text-[#344054] font-DmSans font-medium"
                        >
                          {t("investors.numberOfExits")}
                        </th>
                        <th
                          scope="col"
                          className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                        ...
                        <th
                          ...
                          className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                        >
                          {t("investors.preferredInvestmentIndustry")}
                        </th>
                      </tr>
                    </thead>
                    {!loading &&
                      ...
                      <tbody className="items-center w-full ">
                        {pageData.map((item, index) => (
                          ...
                          >
                            ...
                            <td
                              className="px-[18px] py-4 text-gray500 font-DmSans text-left text-sm font-normal leading-6"
                              style={{ whiteSpace: "nowrap" }}
                            >
                              {t(`${item.type}`)}
                            </td>
                            <td className="px-[18px] py-4 text-center text-gray500 font-dm-sans-regular text-sm leading-6">
                            ...
                            <td
                              className="px-[18px] py-4 text-gray500 font-dm-sans-regular text-sm leading-6"
                              style={{
                                overflow: "hidden",
                                textOverflow: "ellipsis",
                                whiteSpace: "nowrap",
                            ...
                            <td
                              className="px-[18px] py-4 text-gray500 font-dm-sans-regular text-sm leading-6 max-w-[230px] lg:max-w-[250px]"
                              style={{
                                overflow: "hidden",
                                textOverflow: "ellipsis",
                                whiteSpace: "nowrap",
                            ...
                            >
                              {Array.isArray(item?.PreferredInvestmentIndustry)
                                ? item.PreferredInvestmentIndustry.map(
                                    (industry) => t(industry)
                ...
                {!loading &&
                  ...
                  pageData?.length > 0 &&
                  !userDetailsLoading && (
                    <div className="overlay-content-inv w-full flex flex-col top-12 px-8 rounded-b-[8px]"></div>
                  )}
                {!loading &&
                  !subscriptionLoading &&
                  !userDetailsLoading &&
                  ...
                  (!isSubscribe ? (
                    <div className="overlay-content-inv w-full flex flex-col top-12 px-8 rounded-b-[8px]">
                      ...
                      <Text
                        ...
                        size=""
                      >
                        {t("investors.upgradeMessage")}{" "}
                        <a
                          className="text-blue-500"
                          href="/src/Pages/common/Subscription/ChoosePlan"
                        >
                          {t("investors.digitalMoroccoPro1")}
                        </a>{" "}
                        {t("investors.upgradeMessage2")}
                      </Text>
                      <Text
                  ...
                  !userDetailsLoading && (
                    <div className="relative w-full flex items-center p-4">
                      ...
                      {!loading &&
                        ...
                        pageData?.length > 0 &&
                        !userDetailsLoading && (
                          <div className="overlay-content-inv-pg overflow-hidden top-0 rounded-b-[8px]"></div>
                        )}
                      {!loading &&
                        !subscriptionLoading &&
                        !userDetailsLoading &&
                        /* (isSubscribe && userDetails?.projectCount === 0) ||  */
                        (!isSubscribe ? (
                          <div className="overlay-content-inv-pg w-full top-0 rounded-b-[8px]"></div>
                        ) : null)}
                    </div>
                  )}
              </div>
      ...
      <CommonModal
        ...
        title={t("investors.comming.title")}
        showCloseBtn={true}
        content={
          <div className="flex flex-col gap-5 items-center justify-start pb-5 w-full">
            <div className="flex-col justify-center items-center gap-[18px] inline-flex">
              <div className="self-stretch text-center text-[#2575f0] text-base font-dm-sans-medium leading-relaxed">
                {t("investors.comming.header")}
              </div>
              <div className="self-stretch text-center text-[#1d1c21] text-base font-dm-sans-regular leading-relaxed">
                {t("investors.comming.text1")}
              </div>
              <div className="self-stretch text-center text-[#1d1c21] text-base font-dm-sans-medium leading-relaxed">
                {t("investors.comming.text2")}
              </div>
            </div>
            <div className="self-stretch justify-center items-center pt-4 gap-[18px] inline-flex">
              <button
