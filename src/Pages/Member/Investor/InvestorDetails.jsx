...
const InvestorDetails = () => {
  ...
  const [investorInProgressStatus, setInvestorInProgressStatus] =
    useState(false);
  ...
  const {
    data: myInvestments,
    error,
    isFetching: investmentLoading,
    refetch,
  } = useGetAllContactReqByInvestorQuery(queryParams);
  ...
  const getInvestorDetailsRequest = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_baseURL}/investors/${investorId}/details`,
        {
          headers: {
  ...
  return (
    <>
      <HelmetWrapper
        title={t("helmet.investorDetails.title", {
          investorName: investor?.name,
        })}
        description={t("helmet.investorDetails.description", {
          investorName: investor?.name,
        })}
        keywords={t("helmet.investorDetails.keywords")}
        canonical={`${import.meta.env.VITE_URL}/InvestorDetails/${investorId}`}
      />
      <section className="bg-white-A700 flex flex-col gap-8 h-full min-h-screen overflow-auto items-start justify-start pb-14 pt-8 rounded-tl-[40px] w-full">
            ...
            <div className="flex flex-col md:flex-wrap md:flex-row justify-center items-start gap-8">
              <div className="relative flex justify-center w-full min-h-[200px] h-auto md:w-[25%] md:min-w-[250px] max-w-[300px] p-2 border-blue_gray-100 border border-solid rounded-[10px]">
                ...
                {investorRequestStatus?.toLowerCase() === "pending" && (
                  <div className="absolute h-full rounded-[10px] overlay-content-invDetails w-full top-0"></div>
                )}
              </div>
              <div className="flex flex-col md:min-w-[450px] gap-6 flex-1 w-full">
                ...
                >
                  ...
                  >
                    ...
                    {investorRequestStatus?.toLowerCase() === "pending" && (
                      <div className="absolute ml-[-12px] mr-[-10px] h-full overlay-content-invDetails-title w-full top-0"></div>
                    )}
                  </div>
                  {investorRequestStatus?.toLowerCase() !== "accepted" &&
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-7 w-full">
                  <div className="flex flex-col justify-center items-start w-full md:w-[50%] gap-2.5">
                    ...
                    >
                      ...
                      {investorRequestStatus?.toLowerCase() === "pending" && (
                        <div className="absolute h-full overlay-content-invDetails w-full top-0"></div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col justify-center items-start w-full md:w-[50%] gap-2.5">
                    ...
                    <Text className="font-dm-sans-regular text-base leading-6 tracking-wide text-left text-[#344054] pl-8">
                      {investor?.foundedDate
                        ? new Date(investor?.foundedDate).toLocaleString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "short",
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-7 w-full">
                  ...
                  <div className="flex flex-col justify-center items-start flex-1 gap-2.5">
                    ...
                    >
                      ...
                      <IoOpenOutline
                        size={22}
                        className="text-blue-700 cursorpointer"
                        onClick={() =>
                          window.open(
                            investor?.website || "http://venture-catalysts.com",
                            "_blank"
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-7 w-full">
                  <div className="flex flex-col justify-center items-start w-full md:w-[50%] gap-2.5">
                    <div className="flex flex-row gap-3 items-center">
                      ...
                      <Text className="font-dm-sans-bold text-xs leading-4 tracking-wider text-left text-[#98A2B3] uppercase">
                        {t(
                          "investors.investorDetails.preferredInvestmentIndustry"
                        )}
                      </Text>
                    </div>
                    <div className="grid md:flex md:flex-row md:flex-wrap pl-8 gap-[10px]">
                      {investor?.PreferredInvestmentIndustry?.map(
                        (industry, index) => (
                          <div
                            key={index}
                            className="bg-blue-101 w-auto flex justify-center items-center rounded-full px-[14px] h-[30px]"
                          >
                    ...
                    <table className=" w-full">
                      <thead className="">
                        <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[62px]">
                          ...
                          >
                            {t("investors.investorDetails.announcementDate")}
                          </th>
                          <th
                            scope="col"
                            className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                          >
                            {t("investors.investorDetails.companyName")}
                          </th>
                          <th
                            scope="col"
                            className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                          >
                            {t("investors.investorDetails.location")}
                          </th>
                          <th
                            scope="col"
                            className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                          >
                            {t("investors.investorDetails.fundingRound")}
                          </th>
                          <th
                            scope="col"
                            className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                          >
                            {t("investors.investorDetails.moneyRaised")}
                          </th>
                        </tr>
                      </thead>
        ...
        <SendContactModal
          ...
          draftContactId={investorDraftRequest}
          rowData={investor}
          setContactFinalize={setContactSendSuccess}
        />
      </section>
      <CommonModal
        isOpen={showPopup}
        ...
        content={
          <div className="flex flex-col gap-5 items-center justify-start py-5 w-full">
            <div className="self-stretch text-center text-[#1d1c21] text-base font-dm-sans-regular leading-relaxed">
              ...
              <span className="text-[#2575f0]">{t("100 credits")}</span> <br />
