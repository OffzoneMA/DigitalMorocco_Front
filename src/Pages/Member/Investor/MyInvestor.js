...
const MyInvestors = () => {
  ...
  const { data, isFetching: loading, refetch } = useGetInvestorsForMemberQuery(queryParams);
  const { data: locations0, isLoading: locationLoading } = useGetDistinctInvestorFieldValuesQuery('location');
  const { data: locations1, isLoading: location1Loading } = useGetDistinctInvestorFieldValuesQuery('country');
                  ...
                  {filter &&
                    (
                      ...
                      />
                        ...
                        <MultipleSelect className="min-w-[170px] max-w-[250px]" id='investor' options={investorTypes?.distinctValues || []} 
                        searchLabel={t('common.searchType')} setSelectedOptionVal={setLocalInvestmentType}
                          placeholder={t('common.typeofInvestment')} loading={typeLoading}
                          content={
                            (option) => {
                              return (
                        ...
                        <SimpleSelect className="min-w-[100px] max-w-[250px] " id='country' options={locations?.distinctValues || []}
                          searchLabel={t('common.searchLocation')} setSelectedOptionVal={setLocalLocation}
                          placeholder={t("common.location")} loading={locationLoading || location1Loading}
                          content={
                            (option) => {
                              return (
                                <div className="flex  py-2 items-center  w-full">
                        ...
                        <MultipleSelect className="min-w-[170px] max-w-[300px]" id='investor' options={industriesData?.distinctValues || []} 
                        searchLabel={t('common.searchIndustry')} setSelectedOptionVal={setLocalIndustries}
                          placeholder={t('common.selectIndustries')} loading={industryLoading}
                          content={
                            (option) => {
                              return (
                                <div className="flex  py-2 items-center  w-full">
                  ...
                  <table className=" w-full">
                    <thead>
                      <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[44px]">
                        ...
                        <th scope="col" className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium">{t('investors.fundingRound')}</th>
