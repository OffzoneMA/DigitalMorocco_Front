  ...
  return (
      <div className=' py-10 '>
          <h1 className='text-center text-lg md:text-3xl xl:text-5xl font-semibold'>Contact History</h1>
               ...
               <div className='py-10  px-5 md:px-10 xl:px-32 divide-y-2 space-y-2   '>
              ...
              {data?.ContactsHistory?.length == 0 && "Nothing Found !"}
              {
                  ...
                  <div className='space-y-2'>
                                      <div>status: <span className={`${el.status == "accepted" && "text-green-400"} ${el.status == "rejected" && "text-red-400"} ${el.status == "pending" && "text-gray-500"}`}>{el.status}</span></div>
                                      <div>Date: <span className={`italic text-sm text-gray-800`}>{new Date(el.dateCreated).toLocaleDateString('en-US', {
                          weekday: 'long',
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
