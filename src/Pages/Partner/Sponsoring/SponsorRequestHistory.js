...
const SponsorRequestHistory = () => {
  ...
  const { data: distinctValues } = useGetDistinctEventFieldsByPartnerHistoryQuery({ field });
  ...
  const { data: currentRequests, isFetching: isLoading, refetch } = useGetSponsorsHistoryByPartnerQuery(queryParams);
                        ...
                        (pageData.map((item, index) => (
                          ...
                          >
                            <td className="px-[18px] py-4 text-[#667084] font-dm-sans-regular text-sm leading-6">
                              {item?.dateCreated ?
                                <time className="text-[#667084] font-dm-sans-regular text-sm leading-6" dateTime={formatDateValue(item.dateCreated, currentLanguage)}>
                ...
                </table>
                {
                  isLoading ?
                    ...
                    (!pageData?.length > 0 && <div className="flex flex-col items-center text-blue_gray-800_01 gap-[16px] min-h-[330px] w-full py-28 rounded-b-[8px]">
                      <div >
                        <img src={ticketEmptyImg} alt="No Requests" />
                      </div>
