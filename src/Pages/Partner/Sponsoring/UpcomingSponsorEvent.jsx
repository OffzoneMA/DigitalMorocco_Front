...
import {
  ...
  useGetAllUpcomingEventsSponsorsQuery,
...
const UpcomingSponsorEvent = () => {
  ...
  const { data: distinctValues, isLoading: distinctsValueLoading } =
    useGetDistinctValuesByPartnerSponsorQuery(field);
  ...
  const [createSponsor, { isLoading: createLoading, error: createError }] =
  ...
  const {
    ...
    refetch,
  } = useGetAllUpcomingEventsSponsorsQuery(queryParams);
  ...
  const handleCreateSponsor = async (data) => {
    try {
      await createSponsor(data).unwrap();
      refetch();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/UpcomingSponsorEvent`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("event.upcomingEvent")}</PageHeader>
            </div>
                    ...
                    <tbody className="items-center w-full ">
                      {pageData?.map((item, index) => (
                        <tr
                          ...
                          onClick={() =>
                            navigate(`/SponsorEventDetails/${item?._id}`, {
                              state: { event: item },
                            })
                          }
                        >
