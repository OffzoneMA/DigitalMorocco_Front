...
import {
  useGetApprovedSponsorsForPartnerQuery,
...
const PastSponsorEvent = () => {
  ...
  if (filterApply) {
    ...
    queryParams.exactDate =
      selectedDate !== "" ? parseDateString(selectedDate) : "";
  ...
  const {
    ...
    refetch,
  } = useGetApprovedSponsorsForPartnerQuery(queryParams);
  ...
  const filteredData = events?.data?.filter((item) => {
    const keywordMatch = item?.eventId?.title
      ?.toLowerCase()
    ...
    return ReactDOM.createPortal(
      ...
      >
        <div className="mt-4 px-3 py-6 shadow-sm md:shadow-lg bg-white-A700 min-w-40  fex flex-col rounded-md">
          <div
            className="flex flex-row gap-3 items-center cursorpointer hover:text-[#35D8BF]"
            onClick={() =>
              navigate(`/PastSponsorEventDetails/${item?._id}`, {
                state: { event: item },
              })
            }
          >
  ...
  return (
    <>
      <HelmetWrapper
        title={t("helmet.sponsoring.past.title")}
        description={t("helmet.sponsoring.past.description")}
        keywords={t("helmet.sponsoring.past.keywords")}
        canonical={`${import.meta.env.VITE_URL}/PastSponsorEvent`}
      />
                ...
                <div className=" grid-cols-auto-fit md:flex md:flex-1 md:flex-wrap md:flex-row grid grid-cols-2 gap-3 w-auto items-center justify-end ml-auto">
                  {filter && (
                    <>
                      ...
                      <MultipleSelect
                        ...
                        id="typeOfSponsoring"
                        options={sponsoringTypes}
                        searchLabel={t("common.searchType")}
                        setSelectedOptionVal={setTypes}
                  ...
                  {filter ? (
                    ...
                    <button
                      ...
                      } col-span-1 flex flex-row items-center justify-center cursorpointer px-[12px] py-[7px] h-[37px] text-sm font-dm-sans-medium rounded-md`}
                ...
                <table className=" w-full">
                  <thead>
                    <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[44px]">
                      ...
                      >
                        {t("eventListSponsoring.eventName")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.organizedBy")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.date")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.typeOfSponsoring")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.location")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                    ...
                    <tbody className="items-center w-full ">
                      {pageData.map((item, index) => (
                        ...
                        >
                          <td className="w-auto text-gray-801 font-dm-sans-regular text-sm leading-6">
                            <div className="px-[18px] py-4 flex items-center">
                              <img
                                ...
                                className="rounded-md h-[60px] w-[70px] bg-gray-300 mr-3"
                              />
                              <span
                          ...
                          <td
                            className="px-[18px] py-4 text-gray-801 font-dm-sans-regular text-sm leading-6"
                            style={{ whiteSpace: "nowrap" }}
                          >
                            <div className="flex items-center gap-2">
                              {item?.eventId?.organizerLogo ? (
                                <img
                                  src={item?.eventId?.organizerLogo}
                          ...
                          <td className="px-[18px] py-4 text-gray-801 font-dm-sans-regular text-sm leading-6">
                            {item?.sponsorshipType?.toLowerCase() ===
                              "financial" && (
                              <div className="px-[13px] h-[34px] whitespace-nowrap py-2 rounded-[50px] border border-[#6071f3] justify-center items-center gap-1 w-auto inline-flex">
                                <div className="text-[#444ce6] text-xs font-dm-sans-regular leading-none tracking-tight">
                                  {t(item?.sponsorshipType)}
                                </div>
                              </div>
                            )}
                            {item?.sponsorshipType?.toLowerCase() ===
                              ...
                              <div className="h-[34px] px-[13px] whitespace-nowrap py-2 rounded-[50px] border border-[#ffc564] justify-center items-center gap-1 inline-flex">
                                <div className="text-[#e49614] text-xs font-dm-sans-regular leading-none tracking-tight">
                                  {t(item?.sponsorshipType)}
                                </div>
                              </div>
                            )}
                            {item?.sponsorshipType?.toLowerCase() ===
                              ...
                              <div className="h-[34px] px-[13px] whitespace-nowrap py-2 rounded-[50px] border border-[#996fec] justify-center items-center gap-1 inline-flex">
                                <div className="text-[#7f4be7] text-xs font-dm-sans-regular leading-none tracking-tight">
                                  {t(item?.sponsorshipType)}
                                </div>
                              </div>
                            )}
                            {item?.sponsorshipType?.toLowerCase() ===
                              ...
                              <div className="h-[34px] px-[13px] whitespace-nowrap py-2 rounded-[50px] border border-[#24a561] justify-center items-center gap-1 inline-flex">
                                <div className="text-[#028942] text-xs font-dm-sans-regular leading-none tracking-tight">
                                  {t(item?.sponsorshipType)}
                                </div>
                              </div>
                            )}
                          ...
                          >
                            {item?.eventId?.physicalLocation || "Virtual"}
                          </td>
