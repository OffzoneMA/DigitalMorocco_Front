...
import {
  ...
  useRejectSponsorMutation,
  useGetSponsorsByPartnerQuery,
...
const SponsorCurrentRequests = () => {
  ...
  const { data: distinctValues, isLoading: distinctsValueLoading } =
    useGetDistinctEventFieldsByPartnerQuery({ field, eventStatus });
  ...
  const {
    ...
    error: currentRequestsError,
    isFetching: isLoading,
    refetch,
  } = useGetSponsorsByPartnerQuery(queryParams);
  ...
  return (
    <>
      <HelmetWrapper
        title={t("helmet.sponsoring.current.title")}
        description={t("helmet.sponsoring.current.description")}
        keywords={t("helmet.sponsoring.current.keywords")}
        canonical={`${import.meta.env.VITE_URL}/SponsorCurrentRequest`}
      />
        ...
        <div className="flex flex-col items-start justify-start sm:px-5 px-8 w-full">
          <div className="border-b border-gray-201 border-solid flex flex-col md:flex-row gap-5 items-start justify-start pb-6 w-full">
            <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
              <PageHeader>{t("sidebar.sponsoring.currentRequests")}</PageHeader>
            </div>
                  ...
                  {filter && (
                    <>
                      ...
                      <MultipleSelect
                        ...
                        placeholder={t("common.typeRequest")}
                        searchable={false}
                        content={(option) => {
                          return (
                  ...
                  <thead>
                    <tr className="bg-white-A700 text-sm leading-[26px] font-DmSans font-medium h-[44px]">
                      ...
                      <th
                        ...
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.requests")}
                      </th>
                      <th
                        scope="col"
                        className="px-[18px] py-3 text-left text-[#344054] font-DmSans font-medium"
                      >
                        {t("eventListSponsoring.decisionStatus")}
                      </th>
                    </tr>
                  </thead>
                  {pageData?.length > 0 && !isLoading ? (
                    <tbody className="items-center w-full ">
                      {pageData.map((item, index) => (
                        <tr
                          ...
                          onClick={() =>
                            navigate(
                              `/SponsorCurrentRequestDetails/${item?._id}`,
                              { state: { sponsorevent: item } }
                            )
                          }
                        >
                          ...
                          <td className="px-[18px] py-4 text-blue_gray-601 font-dm-sans-regular text-sm leading-6">
                            ...
                            {item?.status?.toLowerCase() === "approved" && (
                              <div className="w-[97px] h-[34px] px-2.5 py-0.5 bg-[#ebfdf2] rounded-2xl justify-center items-center inline-flex">
                                <div className="text-center text-[#027947] text-[13px] font-normal font-['DM Sans'] leading-normal">
                                  {t(item?.status)}
                                </div>
                              </div>
                            )}
                            {item?.status?.toLowerCase() === "rejected" && (
                              <div className="w-[97px] h-[34px] px-2.5 py-0.5 bg-[#fee7e6] rounded-2xl justify-center items-center inline-flex">
                                <div className="text-center text-[#f04437] text-[13px] font-normal font-['DM Sans'] leading-normal">
                                  {t(item?.status)}
                                </div>
                              </div>
                            )}
