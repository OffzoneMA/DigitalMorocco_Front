...
const MyInvestmentDetails = () => {
  ...
  const { data, refetch } = useGetContactRequestByIdQuery(id, { pollingInterval: 3000, refetchOnMountOrArgChange: true, skip: Boolean(!id) });
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_baseURL}/employee/byids`, {
        params: { employeeIds: project?.listMember || [] },
      });
  ...
  useEffect(() => {
    fetchMembers();
  }, [project?.listMember, fetchMembers]);
  ...
  useEffect(() => {
    ...
    // }
    if (project != null) {
      console.log('Project Members:', project.listMember, members, project);
                ...
                <div className="flex gap-y-4 flex-wrap py-2 w-full">
                  <div className="flex flex-col items-start justify-start gap-6 py-2 px-[18px] max-w-full min-w-[150px] basis-[150px]	shrink grow">
                    <div className="bg-white-A700 flex flex-col items-start justify-start w-full">
                      <h2 className="text-[#98A2B3] text-xs tracking-[1.68px] uppercase font-dm-sans-bold" size="txtDMSansBold12">
                  ...
                  <div className="flex flex-col items-start justify-start px-[18px] py-2 gap-6 max-w-full min-w-[150px] basis-[150px]	shrink grow">
                    ...
                    <div className="flex flex-row items-start justify-start w-full">
                      <div className={`flex items-center justify-center text-center w-auto px-[10px] py-[4px] h-[28px] gap-[6px] font-inter text-sm font-medium leading-[20px] rounded-full ${project?.status === 'Active' ? 'bg-green-100 text-green-700' :
                  ...
                  {project?.logo &&
                    <div className="flex flex-col gap-[10px] pb-[41px] items-start justify-start w-full">
                      ...
                      <div className="h-[270px] w-full max-w-[500px] rounded-[6px] px-3 py-[50px] justify-center border border-[#D0D5DD] items-center gap-1.5 flex">
                        <img src={project?.logo} alt="Logo" className="rounded-[6px] h-[270px] w-auto" />
                      </div>
                    </div>}
