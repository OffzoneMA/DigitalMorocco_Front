...
const MyInvestment = () => {
  ...
  const { data: sectorData, isLoading: locationLoading } =
    useGetDistinctProjectFieldsQuery({ field: "status", status: "Approved" });
  const { data: fundingData, isLoading: industryLoading } =
    useGetDistinctProjectFieldsQuery({ field: "funding", status: "Approved" });
  const { data: locationData, isLoading: typeLoading } =
    useGetDistinctProjectFieldsQuery({ field: "stage", status: "Approved" });
  ...
  return (
    <>
      <HelmetWrapper
        ...
        canonical={`${import.meta.env.VITE_URL}/MyInvestment`}
      />
                    ...
                    <tbody className="items-center w-full ">
                      {filteredData.map((item, index) => (
                        <tr
                          ...
                          onClick={() =>
                            navigate(`/InvestmentDetails/${item?._id}`, {
                        ...
                        >
                          ...
                          <td className="px-[18px] py-4 text-blue_gray-601 font-dm-sans-regular text-sm leading-6">
                            {item?.project?.stage
                              ? t(item?.project?.stage)
                              : "-"}
                          </td>
                          <td className="px-[18px] py-4 text-blue_gray-601 font-dm-sans-regular text-sm leading-6">
                            {item?.project?.milestones?.[0]?.name || "-"}
                          </td>
                          <td className="px-[18px] py-4 text-blue_gray-601 font-dm-sans-regular text-sm leading-6">
                            ...
                            <StatusBadge
                              status={item?.project?.status || "In Progress"}
                            />
                          </td>
                        </tr>
                      ))}
