...
export default function UserProfile() {
  ...
  const onSubmit1 = async (data) => {
    try {
      ...
      updatedFields.displayName = `${data.firstName} ${data.lastName}`;
      ...
      updatedFields.country = selectedCountryName || currentData.country;
      updatedFields.cityState = selectedCityName || currentData.cityState;
  
      const fields = ['facebook', 'instagram', 'twitter', 'linkedin', 'language', 'region'];
      fields.forEach(field => {
        updatedFields[field] = data[field] || currentData[field];
      });
...
<div className='flex flex-1 flex-col gap-5'>
  <form onSubmit={handleSubmit1(onSubmit1)} className='flex w-full flex-col gap-5 border-b border-indigo-50 border-solid pb-8'>
    <div className='flex flex-row w-full gap-5'>
      <div className={`flex flex-col gap-2 items-start justify-start  w-full`}>
        ...
        {errors1.firstName && <span className="text-sm font-DmSans text-red-500">
          {errors1.firstName?.message}</span>}
      </div>
      <div className={`flex flex-col gap-2 items-start justify-start w-full`}>
        <Text className="text-base text-gray-901 w-auto" size="txtDMSansLablel" >
          Last Name
        ...
        {errors1.lastName && <span className="text-sm font-DmSans text-red-500">
          {errors1.lastName?.message}</span>}
      </div>
    </div>
    <div className='flex flex-row w-full gap-5'>
      <div className={`flex flex-col gap-2 items-start justify-start  w-full`}>
        ...
        {errors1.email && <span className="text-sm font-DmSans text-red-500">
      ...
      <div className="flex md:flex-1 w-full md:w-full rounded-md p-2 border border-gray-301">
        ...
        </div> {errors1.phoneNumber && <span className="text-sm font-DmSans text-red-500">
        {errors1.phoneNumber?.message}</span>} 
        </div> </div> 
        <div className='flex flex-row w-full gap-5'> 
  ...
  <div className={`flex flex-col gap-2 items-start justify-start w-full`}> 
    ...
    <div className="flex md:flex-1 w-full md:w-full rounded-md p-2 border border-gray-301">
      <input {...register1('website', { required: { value: false, message: 'Website is required' } })} 
      className={`!placeholder:text-blue_gray-301 !text-gray700 font-manrope p-0 text-left text-sm tracking-[0.14px] w-full bg-transparent border-0`} 
  ...
  <div className={`flex flex-col gap-2 items-start justify-start w-full`}>
  <Text className="text-base text-gray-901 w-auto" size="txtDMSansLablel" > Address </Text> 
  <div className="flex md:flex-1 w-full md:w-full rounded-md p-2 border border-gray-301">
    <input {...register1('address', { required: { value: true, message: 'Address is required' } })} 
    className={`!placeholder:text-blue_gray-301 !text-gray700 font-manrope p-0 text-left text-sm tracking-[0.14px] w-full bg-transparent border-0`} 
    type="text" name="address" placeholder="Enter Address" /> </div>
    {errors1.address && <span className="text-sm font-DmSans text-red-500">
    {errors1.address?.message}</span>} </div> </div> 
    <div className='flex flex-row w-full gap-5'> 
    <div className={`flex flex-col gap-2 items-start justify-start  w-full`}>
      <Text className="text-base text-gray-901 w-auto" size="txtDMSansLablel" > 
      Country </Text> 
      <SimpleSelect id='country' options={allCountries} onSelect={""} searchLabel='Select Country' 
      setSelectedOptionVal={setSelectedCountry} placeholder={
        selectedCountry ? selectedCountry : "Select Country"
    } 
  
      valuekey="name" content={(option) => { return (
      <div className="flex  py-2 items-center  w-full">
        <Text className="text-gray-801 text-left text-base font-DmSans font-normal leading-5 w-auto" > 
        {option.name} </Text> </div>); }} /> 
        {/* <SelectPicker size="md" data={allCountries} labelKey='name' valueKey='isoCode' value={selectedCountry} onChange={setSelectedCountry} menuClassName="w-auto" className="w-full z-0 !placeholder:text-blue_gray-301 !text-gray700 font-manrope font-normal leading-18 tracking-wide" placeholder="Select Country" renderMenuItem={(label, item) =>{ return ( <div className="flex items-center justify-start space-x-3"> <div className="flex flex-col gap-2 justify-center w-full"> <Text className="text-gray-801 text-base font-DmSans font-normal leading-5 w-auto" > {item.name} </Text> </div> </div> ); } }/> */} 
        </div> 
        <div className={`flex flex-col gap-2 items-start justify-start w-full`}>
        <Text className="text-base text-gray-901 w-auto" size="txtDMSansLablel" >
          City/State </Text> 
          ...
          placeholder={
           ...
           valuekey="name" content={(option) => { return (<div className="flex  py-2 items-center  w-full"> 
          <Text className="text-gray-801 text-left text-base font-DmSans font-normal leading-5 w-auto" >
          {option.name} </Text> </div>); }} /> 
          {/* <SelectPicker size="md" data={City.getCitiesOfCountry(selectedCountry)} valueKey="name" labelKey='name' value={selectedCity} onChange={setSelectedCity} menuClassName="w-auto" className="w-full z-0 !placeholder:text-blue_gray-301 !text-gray700 font-manrope font-normal leading-18 tracking-wide" placeholder="Select City" renderMenuItem={(label, item) =>{ return ( <div className="flex items-center justify-start space-x-3"> <div className="flex flex-col gap-2 justify-center w-full"> <Text className="text-gray-801 text-base font-DmSans font-normal leading-5 w-auto" > {item.name} </Text> </div> </div> ); } }/> */}
            </div> </div> 
            {!isForm1Saved ? (
            <button className="bg-blue-A400 font-DmSans font-medium text-white-A700 flex flex-row md:h-auto items-center mr-auto py-2 px-10 rounded-md w-auto" type="submit" >
              ...
              <SlCheck size={20} />
                <span className="text-base text-gray500">Saved</span> </button>
                )
            } 
            </form> 
            ...
            <button className="text-blue-501 flex flex-row items-center justify-center py-2 px-4 gap-3 rounded-md border-[1.3px] border-blue-501" type="submit" >
              ...
              Save </button> </div>
                ...
                <Text className="text-base text-gray-901 w-[130px] " size="txtDMSansLablel" >
                  ...
                  setSelectedOptionVal={setSelectedRegion} placeholder="Select Your Region" valuekey="label" 
                  ...
                  {/* <SelectPicker size="md" data={regions} valueKey='label' value={selectedRegion} onChange={setSelectedRegion} menuClassName="w-auto" className="w-[40%] z-0 !placeholder:text-blue_gray-301 !text-gray700 font-manrope font-normal leading-18 tracking-wide" placeholder="Select Your Region"/> */}
                    ...
                    <DeleteAccountModal isOpen={isDeleteModalOpen} onRequestClose={closeDeleteModal} /> 
                    </div>
);
}