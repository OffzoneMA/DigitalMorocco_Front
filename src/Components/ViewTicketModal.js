    ...
    if (startDate || !isValid(startDate)) {
      ...
      const formattedStartDate =
        startDate && isValid(startDate)
          ? format(
              startDate,
              currentLanguage === "fr" ? "eee dd MMM yyyy" : "eee, MMM d, yyyy",
      ...
      return currentLanguage === "fr"
        ? `${capitalizeFirstLetter(
            formattedStartDate.replace(".", "")
          )?.replace(".", "")} à ${formatTime(startTime)}`
        : `${capitalizeFirstLetter(
            formattedStartDate.replace(".", "")
          )?.replace(".", "")} ${formatTime(startTime)}`;
    }
  };
    ...
    >
      <div className="max-h-[97vh] w-full md:w-full">
        <div className="bg-white-A700 border border-gray-500_33 max-h-[97vh] overflow-y-auto border-solid flex flex-col gap-6 items-center justify-start max-w-screen-sm  rounded-[10px] w-full">
          <div
            className="h-36 pb-6 w-full flex flex-row items-start rounded-t-[10px]"
            style={{
              backgroundImage: `url(${rowData?.headerImage})`,
              backgroundSize: "100%",
              backgroundRepeat: "no-repeat",
            }}
          >
            <div
              className="hover:bg-gray-201 w-auto rounded-full p-1 justify-end ml-auto"
              onClick={props.onRequestClose}
            >
              <IoCloseOutline className="text-white-A700" size={20} />
            </div>
          </div>
          <div className="flex flex-col w-full px-3 gap-3">
            <h1 className="font-dm-sans-medium text-lg leading-7 text-center text-blue_gray-900_03 w-full">
              {rowData?.title}
            </h1>
            <div className="flex flex-row gap-3 items-center justify-center">
              ...
              <time
                dateTime={
                  rowData?.startDate
                    ? new Date(rowData.startDate).toISOString()
                    : ""
                }
            ...
            <div className="flex flex-row gap-3 items-center justify-center">
              ...
              <p className="text-blue_gray-601 font-dm-sans-regular text-base leading-6">
            ...
            <div className="flex flex-row px-12 items-center justify-center">
              <p className="text-blue_gray-601 font-DmSans text-center text-xs font-normal leading-[19.2px]">
                {t("event.termsConditionsStart")}
                <a
                  className="text-blue-A400"
                  href={`https://digitalmorocco.net/terms?lang=${currentLanguage}`}
                >
                  {t("event.termsConditionsLink")}
                </a>{" "}
                {t("event.termsConditionsEnd")}
              </p>
            </div>
