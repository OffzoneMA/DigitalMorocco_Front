    ...
    <>
      {(userLastPaymentMethod !== null) ?
        <div className="flex flex-col w-full gap-6">
          ...
          <div className="flex flex-row w-full rounded-[12px] border py-4 px-4 border-gray-301 gap-4">
            <div className="flex rounded-md px-3 py-3.5 bg-gray-201 items-center">
              <img src={selectedMethod?.image || "images/img_visa.svg"} alt="payment method logo" />
            </div>
            <div className="flex flex-col gap-3">
              <h2 className="font-dm-sans-medium text-sm leading-[18px] text-left text-[#101828] w-full">
                {userLastPaymentMethod?.cardNumber ? formatCardNumberForDisplay(userLastPaymentMethod?.cardNumber) : '•••• •••• •••• ••••'}
              </h2>
              <h3 className="font-dm-sans-regular text-xs leading-[15.62px] text-gray500 text-left w-full">
        ...
        <div className="flex flex-col w-full gap-7">
          <p className="font-dm-sans-medium text-lg leading-7 text-[#101828] text-left w-full">
            {t('payment.paymentInfo')}
          </p>
          <button
