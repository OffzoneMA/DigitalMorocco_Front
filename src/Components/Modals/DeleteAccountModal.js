        ...
        <div className="bg-white-A700 border border-gray-500_33 max-h-[97vh] overflow-y-auto border-solid flex flex-col p-6 gap-4 items-center justify-start max-w-screen-sm  rounded-[10px] w-full">
          <div className="flex flex-col w-full gap-3">
            <div className="flex flex-row w-full gap-2 items-center">
              ...
              <h1 className="font-dm-sans-medium text-[20px] leading-6 text-[#E02D3C] w-full">{t('settings.deleteAccount.confirmationMessage')}</h1>
            </div>
            <p className="font-dm-sans-regular text-base leading-6 text-[#1D1C21] text-left w-full">
              {t('settings.deleteAccount.warningMessage')} <span className="font-dm-sans-medium">{t('settings.deleteAccount.warningMessage1')}</span>
            </p>
            <div className="flex flex-col w-full gap-3">
              <p className="font-dm-sans-regular text-base leading-6 text-[#1D1C21] text-left w-full">{t('settings.deleteAccount.deletionNotice')}</p>
              <ul className="list-disc list-inside space-y-2 font-dm-sans-regular text-base leading-6 text-[#1D1C21] text-left">
                <li>{t('settings.deleteAccount.dataDeleted.data1')}</li>
                <li>{t('settings.deleteAccount.dataDeleted.data2')}</li>
                <li>{t('settings.deleteAccount.dataDeleted.data3')}</li>
              </ul>
              <p className="font-dm-sans-regular text-base leading-6 text-[#1D1C21] text-left w-full">{t('settings.deleteAccount.emailPrompt')}</p>
              <div className={`flex flex-row gap-14 items-center justify-start pt-3 w-full`}>
                <Text htmlFor="new_email" className={`text-base text-[#1D1C21] ${currentLanguage === 'fr' ? 'w-[150px]' : 'w-[100px]'} `} size="txtDMSansLablel">{t('settings.deleteAccount.email')}</Text>
                <input
                  {...register('new_email',
                    {
              ...
              <div className={`flex flex-row gap-14 items-center justify-start pt-3 w-full`}>
                <Text htmlFor="new_password" className={`text-base text-[#1D1C21] ${currentLanguage === 'fr' ? 'w-[150px]' : 'w-[100px]'} `} size="txtDMSansLablel">{t('settings.deleteAccount.password')}</Text>
