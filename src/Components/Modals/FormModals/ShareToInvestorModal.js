  ...
  const handleInvestorSelection = (id) => {
    ...
    if (investorsData?.investors && Array.isArray(investorsData.investors)) {
      ...
      const newSelectedNames = filteredInvestors.map(
        (investor) =>
          investor?.name || investor?.companyName || "Unnamed Investor"
      );
      ...
      console.error(
        "Investors data is not properly defined or is not an array:",
        investorsData
      );
    }
  };
    ...
    <>
      ...
      >
        <div className="max-h-[93vh] sm:w-full md:w-full">
          <div
            className={`bg-white-A700 border border-gray-500_33 border-solid flex flex-col gap-6 items-center justify-start max-w-screen-sm px-3 md:px-5 pt-5 pb-[30px] rounded-[8px] w-full`}
          >
            ...
            {!isLoading && investorsData?.investors?.length === 0 ? (
              <div className="flex-col justify-center items-center inline-flex">
                ...
                <div className="self-stretch h-[35px] mt-[32px] text-center text-[#344053] text-sm font-dm-sans-medium leading-relaxed">
                  {t("projects.shareProject.emptyInvestorsList.mainMessage")}
                </div>
                <div className="self-stretch text-center text-[#344053] px-3 text-sm font-dm-sans-regular leading-relaxed">
                  {t("projects.shareProject.emptyInvestorsList.instruction")}
                </div>
              </div>
            ) : (
                  ...
                  {isLoading ? (
                    ...
                    filteredInvestors?.map((item, index) => (
                      ...
                      >
                        ...
                        <p
                          className="text-sm capitalize text-gray-900_01 leading-6 tracking-normal font-dm-sans-regular"
                        >
                          {item.name}
                        </p>
                      </div>
                ...
                <>
                  <div className="justify-end items-start gap-[18px] inline-flex">
                    <button
                      type="button"
                      onClick={props.onRequestClose}
                      className="px-5 cursorpointer py-[18px] h-[44px] bg-[#e4e6eb] hover:bg-[#D0D5DD] active:bg-light_blue-100 rounded-md justify-center items-center gap-[18px] flex"
                    >
                    ...
                    <button
                      ...
                      className="px-5 cursorpointer py-[18px] h-[44px] bg-[#2575f0] hover:bg-[#235DBD] active:bg-[#224a94] rounded-md justify-center items-center gap-[18px] flex"
                    >
                      <div className="justify-center items-center gap-3 flex">
                        <div className="text-white-A700 text-base font-dm-sans-medium">
                          {t(
                            "projects.shareProject.emptyInvestorsList.buttonText"
                          )}
                        </div>
                      </div>
                    </button>
