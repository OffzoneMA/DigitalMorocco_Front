  ...
  const onSubmit = async (data) => {
    if (typeInvestment !== null) {
      try {
        setSendingOk(true);
        await props?.methode({
          approvalLetter: data?.letter,
          sponsorshipType: typeInvestment,
        });
        openModal();
      } catch (error) {
    ...
    <>
      ...
      >
        <form
          ...
          className="max-h-screen w-full md:w-full"
        >
          <div className="bg-white-A700 border border-gray-500_33 border-solid flex flex-col gap-6 items-center justify-start max-w-screen-sm py-6 rounded-[10px] w-full">
            <div className="flex flex-row gap-5 items-start justify-start px-5 w-full">
              <div className="flex flex-1 flex-col font-DmSans h-full items-start justify-start w-full">
                <h1 className="font-DmSans md:text-lg text-[18px] leading-7 font-medium text-[#1D2939] w-full">
                  {t("eventListSponsoring.approve.title")}
                </h1>
              </div>
            </div>
            <div className="flex px-6 md:px-5 h-[1px] w-full">
              {" "}
              <div className="bg-gray-201 w-full"></div>
            </div>
            <div className="flex flex-col gap-5 w-full max-h-[70vh] px-5 overflow-y-auto">
              ...
              >
                ...
                >
                  {t(
                    "eventListSponsoring.sponsorshipRequest.sponsoringTypeLabel"
                  )}
                </Text>
                <SimpleSelect
                  id="project"
                  options={typesOfSponsoring}
                  searchLabel={t("common.searchType")}
                  setSelectedOptionVal={setSelectedInvestmentType}
                  placeholder={t("eventListSponsoring.approve.sponsoringType")}
                  required={sending && typeInvestment === null}
                  content={(option) => {
                    return (
              ...
              >
                <Text
                  ...
                  htmlFor="letter"
                >
                  {t("eventListSponsoring.approve.letterLabel")}
                </Text>
                <textarea
                  {...register("letter", {
                    required: {
                      value: true,
                      message: "Request Letter is required.",
                    },
                  })}
                  id="letter"
                  name="letter"
                  className={`!placeholder:text-blue_gray-301 !text-gray700 h-[139px] leading-[18.2px] font-manrope text-left text-sm tracking-[0.14px] w-full rounded-[6px] px-[12px] py-[10px]  border border-[#D0D5DD] ${
                    errors?.letter
                      ? "border-errorColor shadow-inputBsError focus:border-errorColor"
                      : "border-[#D0D5DD] focus:border-focusColor focus:shadow-inputBs"
                  }`}
                  rows={5}
                  placeholder={t("eventListSponsoring.approve.requestLetter")}
                  style={{
                    scrollbarWidth: "none",
                ...
                >
                  {t("eventListSponsoring.approve.companyIntroduction")}
                </p>
              </div>
            </div>
            <div className="flex space-x-3 md:space-x-5 items-end w-full px-5 justify-end">
              ...
              <button
                ...
                } hover:bg-[#235DBD] active:bg-[#224a94] text-white-A700 py-[10px] md:py-[20px] px-[12px] md:px-[20px] font-dm-sans-medium text-base h-[44px] leading-5 tracking-normal rounded-[6px] cursorpointer-green`}
              >
                {sendingOk ? (
