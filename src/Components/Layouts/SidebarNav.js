...
const SidebarNav = () => {
  ...
  const { data: userDetails, isLoading: userDetailsLoading, refetch } = useGetUserDetailsQuery();
  const { data: notifications, refetch: refetchNotifications } = useGetNotificationSummaryQuery();
  ...
  const Menus = useMemo(() => [
    { title: t('sidebar.dashboard'), src: <RiHome6Line size={23} />, link: userRole?.toLowerCase() === "admin" ? "Dashboard_Admin" : userRole?.toLowerCase() === "investor" ? "Dashboard_Investor" : userRole?.toLowerCase() === "partner" ? "Dashboard_Partner" : "Dashboard" },

    (userRole === "member") && { title: t('sidebar.projects'), src: <GoRocket size={23} />, link: "Projects", activeLinks: ["Projects", "CreateProject", "EditProject", "Projectdetails"] },
    (userRole === "investor") && { title: t('sidebar.myProfile'), src: <BiBuildings size={23} />, link: "InvestorProfile", activeLinks: ["InvestorProfile"] },
    (userRole === "member" || userRole === "investor" || userRole === "partner") && {
      title: t('sidebar.company.main'),
      src: <BiBuildings size={23} />,
      submenu: true,
    ...
    ,
    (userRole === "member") && {
      title: t('sidebar.investor.main'), src: <TiFlashOutline size={23} />,
    ...
    (userRole === "investor" && validEmailCheck) && {
      title: t('sidebar.investment.main'), src: <TiFlashOutline size={23} />,
    ...
    (userRole === "member" || validEmailCheck) && {
      title: t('sidebar.event.main'), src: <HiOutlineTicket size={23} />,
    ...
    (userRole === "partner" && validEmailCheck) && {
      title: t('sidebar.sponsoring.main'), src: <PiAtomBold size={23} />,
    ...
    (userRole === "member" || validEmailCheck) && { title: t('sidebar.document'), src: <PiFolderThin size={23} />, link: "Document" },
    (userRole === "admin") && { title: t('sidebar.users'), src: <RiUser3Line size={23} />, link: "Users" },
    (userRole === "member" || validEmailCheck) && { title: t('sidebar.history'), src: <PiHourglassLowFill size={23} />, link: "History" },
    (userRole === "admin") && {
      title: t('sidebar.adminHistory.main'),
      src: <PiHourglassLowFill size={23} />,
      submenu: true,
  ...
  const isSubmenuOpenWithoutActiveChild = (menu) => {
    const isSubmenuOpen = submenuOpen[menu.title];
    const hasActiveChild = menu.child?.some(child =>
      activeMenu === child.link || child.activeLinks?.includes(activeMenu)
    );
    return isSubmenuOpen && !hasActiveChild;
  };

  return (
    <aside className={`bg-blue_gray-901 flex flex-col h-full min-h-screen pt-8 ${open ? "w-[280px]" : "w-20"} duration-300 relative no-scrollbar-div`}>
      ...
      <div className={`flex flex-col overflow-y-auto flex-1 h-full ${open ? "w-auto" : "w-20"} w-full pb-5 px-5`}>
          ...
          {Menus.map((Menu, index) => (
            Menu && <div key={index} >
              <li
                ...
                className={`overflow-x-visible border-l-2 border-transparent ${!open && 'w-fit'} group flex rounded-md p-2 cursorpointer hover:bg-blue_gray-902 hover:text-teal-400 ${(activeMenu === Menu.link || Menu.activeLinks?.includes(activeMenu)) ? "bg-blue_gray-902 text-teal-400" : isSubmenuOpenWithoutActiveChild(Menu) ? "bg-blue_gray-902/50 border-teal-400/70" : "hover-active-color"} text-gray-301 items-center ${open ? "gap-x-3" : "gap-x-1.5"} mt-3 transition-all duration-300`}
                onMouseEnter={(e) => handleItemMouseEnter(e, Menu.title)}
                onMouseLeave={handleItemMouseLeave}
              // title={!open ? Menu.title : ""}
              >
                <span className={`duration-200 ${(activeMenu === Menu.link || Menu.activeLinks?.includes(activeMenu)) ? "active-icon-color" : ""}`}>
                  {Menu.src}
                </span>
              ...
              <ul className={`overflow-hidden transition-all duration-300 ease-in-out ${submenuOpen[Menu.title]
                ...
                {Menu.submenu && (
                  Menu.child.map((el, i) => (
                    <li
                      ...
                      className={`group flex text-base font-dm-sans-regular leading-6 ${!open && 'w-full'} rounded-md py-2 pl-10 cursorpointer hover:bg-blue_gray-902 hover:text-teal-400 ${(activeMenu === el.link || el.activeLinks?.includes(activeMenu)) ? "bg-blue_gray-902 text-teal-400" : ""} text-gray-301 items-center gap-x-2  mt-1 transition-all duration-300`}
        ...
        <div className=" text-base font-dm-sans-regular leading-6 mb-[50px]">
          <div
            ...
            className={` ${!open && 'w-fit'} group flex ${!settingsOpen && 'mb-4'} rounded-md p-2 cursorpointer ${(activeMenu === "settings" || settingActiveLinks?.includes(activeMenu)) ? "bg-blue_gray-902 text-teal-400" : (settingsOpen && !settingActiveLinks?.includes(activeMenu)) ? "bg-blue_gray-902/50" : "hover-active-color"} text-gray-301 items-center ${open ? "gap-x-3" : "gap-x-1.5"} hover:bg-blue_gray-902 hover:text-teal-400 text-gray-301 items-center  gap-x-3 mt-3 `}
