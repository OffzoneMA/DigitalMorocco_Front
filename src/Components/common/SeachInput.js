  ...
  const highlightText = (text) => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === searchValue.toLowerCase() ? (
        <span key={index} style={{ color: '#35D8BF'}}>{part}</span>
      ) : (
        part
      )
    );
              ...
              <li key={index} className="py-2 text-[12px] border-b border-gray-201 text-[#667085] w-full flex flex-col font-dm-sans-regular">
                ...
                <ul>
                  {category?.results?.map((result) => (
                    <li key={result._id} className="py-1 text-[13px] text-gray700 px-1 font-dm-sans-medium cursorpointer hover:text-[#35D8BF] capitalize" 
                    onClick={() => handleRedirect(category?.label , result._id)}>
                      {highlightText(result?.name || result?.title || result?.fullName || result?.companyName || result?.investor?.name || result?.member?.companyName || `${historyEventMessages[result?.eventType]} ${result?.eventData?.targetName}`)}
                    </li>
                  ))}
                </ul>
              </li>
